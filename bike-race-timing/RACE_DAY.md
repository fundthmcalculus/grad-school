...
- [ ] **Browser: Chrome or Edge, recent version.** These give you WebGPU
      preprocessing and the crash-safe live-save folder. Firefox/Safari will
      still work (OCR + recording both run fine), just without those two
      extras — footage is held in memory and only downloaded at the end.
...
5. Tune **Contrast / Brightness / Binarize / Threshold** while watching the
   "WebGPU preprocessed" preview — you want the bib numbers to look like
   clean, high-contrast dark-on-light (or light-on-dark) digits.
6. **Start Session.** Confirm the status line says "Session running" and the
   ...
   preview panels.
7. Let it run. The live log table (right side) fills in as riders are
   confirmed; a row briefly highlights when it's added.
...
| No riders ever appear in the live log | ROI probably doesn't cover where numbers are legible, or contrast is too low — check the two preview panels; the "WebGPU preprocessed" one should look like clean, high-contrast digits, not a grey blur. |
| Numbers logged, but often wrong digits | Increase Contrast, try Binarize, or reduce Min confidence temporarily to see what OCR is guessing — then retune. Faded/dirty bibs or motion blur will always cause some misses; that's what Review is for. |
...
| Laptop fan spinning hard / getting warm | Expected — camera capture + WebGPU + continuous OCR + video encoding is real, sustained work. Make sure it's not also on a lap blocking airflow. |
