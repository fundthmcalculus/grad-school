...
def build():
    ...
    ax.text(2, 95, "Every arm shares the same outer loop", ha="left", va="center",
            fontsize=F.FS_LABEL, color=F.INK, fontweight="bold")
    outer = [(17, "Seed", "the farther endpoint of\nthe most-dissimilar pair"),
             (50, "Repeat $N-1$ times", "append the nearest unplaced point\n"
                                        "to the ordered set"),
             (84, "VAT ordering", "identical across all three arms,\n"
                                  "checked bit for bit")]
    for x, title, body in outer:
        F.box(ax, x, 82, 30, 14, title, body, color=F.FAINT, fill_amount=0.93,
    ...
    ax.text(50, 67.5, "What differs is the step inside it — how the next minimum is found",
            ha="center", va="center", fontsize=F.FS_LABEL, color=F.INK,
            fontweight="bold")

    arms = [
        (54, F.ORANGE, "classical", "$O(N^3)$",
        ...
        (36, F.BLUE, "stage one — published", "$O(N^2\\log N)$",
         "Priority queue with\nlazy deletion",
        ...
        (18, F.AQUA, "stage two — unpublished", "$O(N^2)$",
    ...
    for y, color, name, order, headline, detail in arms:
        F.box(ax, 18, y, 30, 13, headline, None, color=color, fill_amount=0.88,
              title_size=F.FS_SMALL)
        ax.text(35, y, order, ha="left", va="center", fontsize=F.FS_LABEL,
                color=F.shade(color, 0.25), fontweight="bold")
        ax.text(50, y + 2.6, name, ha="left", va="center", fontsize=F.FS_SMALL,
                color=F.shade(color, 0.25))
        ax.text(50, y - 2.4, detail, ha="left", va="center",
                fontsize=F.FS_SMALL - 0.5, color=F.MUTED, linespacing=1.5)

    ax.plot([2, 98], [8, 8], lw=0.8, color=F.GRID)
    ax.text(2, 3, "Both fast arms ship, deliberately: stage one is the portable "
