...
def build():
    ...
    for init, colour in (("hot", HOT_C), ("cold", COLD_C)):
        ...
        if per_arm:
            rx.plot(
                budgets,
                np.median(np.vstack(per_arm), axis=0),
                lw=2.4,
                color=colour,
                marker="o",
                ms=4.5,
    ...
    F.style_axes(
        rx,
        title="(a)  held-out $R^2$ against budget",
    ...
    for arm in present:
        vals = [
            int(r["evals_to_heuristic"])
            for r in seeds_by.get((arm, "cold"), [])
    ...
    px.barh(
        y,
        [max(reach[a][0], 1.0) for a in order],
    ...
    F.style_axes(
        px,
        title="(b)  cold start: evaluations to\nmatch the construction",
        xlabel="objective evaluations (log)",
        grid_axis="x",
    )
    ...
    for k, init in enumerate(inits):
        ...
        for arm in present:
            d = [
                float(r["r2"]) - float(r["heuristic_r2"])
                for r in seeds_by.get((arm, init), [])
                if r.get("heuristic_r2")
            ]
        ...
        bx.barh(
            ...
            xerr=sds,
            height=width * 0.9,
            color=[F.tint(COLOUR[a], 0.0 if init == "hot" else 0.55) for a in present],
            zorder=3,
    ...
    F.style_axes(
        bx,
        title="(c)  paired gain over the\nconstruction, ±1 s.d.",
        xlabel="$R^2$(run) $-$ $R^2$(heuristic)",
        grid_axis="x",
    )
    bx.text(
        0.0,
        1.005,
        "solid = hot start · pale = cold start",
