...
def build():
    ...
    ax.text(LEFT, 97, "Conventional: search first", ha="center", va="center",
            fontsize=F.FS_TITLE, color=F.INK, fontweight="bold")
    ax.text(RIGHT, 97, "This work: structure first", ha="center",
            va="center", fontsize=F.FS_TITLE, color=F.INK, fontweight="bold")
    ...
    left_stack = [
        (88, 14, "Grid-partition every input",
         f"{SETS_PER_FEATURE} fuzzy sets on each of $M$ features"),
        (64, 22, "Search the whole model",
    ...
    # -- right column --------------------------------------------------------
    right_stack = [
        (88, 14, "Recover the structure",
         "mergeVAT reorder, persistence\n(Ch. 3, Ch. 5)"),
        (71, 14, "Read the model off it",
         "per-class Gaussian mixtures,\nclosed-form consequents   (Ch. 4)"),
        (55, 14, "Polish locally — optional",
    ...
    ax.text(1, 23, "rule\nbase", ha="left", va="center", fontsize=F.FS_SMALL,
            color=F.MUTED, linespacing=1.4)
    ax.text(1, 7, "training\ntime", ha="left", va="center", fontsize=F.FS_SMALL,
            color=F.MUTED, linespacing=1.4)

    ax.text(LEFT, 23, f"${mantissa:.1f}\\times10^{{{exponent}}}$ rules",
            ha="center", va="center", fontsize=13,
            color=F.shade(F.ORANGE, 0.2), fontweight="bold")
    ax.text(LEFT, 16, f"$\\prod_i N_{{\\mu_i}} = {SETS_PER_FEATURE}^{{{N_FEATURES}}}$"
                      f"  on PhiUSIIL's {N_FEATURES} features",
            ha="center", va="center", fontsize=F.FS_SMALL, color=F.MUTED)
    ax.text(RIGHT, 23, f"{N_CLASSES} rules", ha="center", va="center",
            fontsize=13, color=F.shade(F.BLUE, 0.2), fontweight="bold")
    ax.text(RIGHT, 16, f"one per answer, $K = {N_CLASSES}$", ha="center",
            va="center", fontsize=F.FS_SMALL, color=F.MUTED)

    ax.plot([1, 99], [12, 12], lw=0.8, color=F.GRID, zorder=1)
    ax.text(LEFT, 7, "no measured baseline — the ANFIS and GA-FIS\n"
                     "adapters are Goal G3 (Table 4.1 reports N/A)",
            ha="center", va="center", fontsize=F.FS_SMALL, color=F.MUTED,
            linespacing=1.5, style="italic")
    ax.text(RIGHT, 7, f"{train_s} s   ·   accuracy {accuracy:.3f}",
            ha="center", va="center", fontsize=F.FS_ANNOT,
            color=F.shade(F.BLUE, 0.2))

    ax.text(99, 0.5, H.provenance_note(label), ha="right", va="center",
