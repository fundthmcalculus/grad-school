...
def build():
    ...
    fig, (curve_ax, part_ax) = F.grid_figure(1, 2, width=F.W_WIDE, height=3.4,
    ...
    grid = np.concatenate([np.linspace(0, death * 2, 500),
                           np.geomspace(death * 2, d.max() * 1.15, 300)])
    ramp = np.clip((death - grid) / (death - birth), 0.0, 1.0)
    gaussian = np.exp(-np.log(2.0) * (grid / death) ** 2)

    curve_ax.plot(grid, ramp, lw=2.0, color=F.BLUE,
                  label="persistence ramp, §5.3.4")
    curve_ax.plot(grid, gaussian, lw=2.0, ls=(0, (4, 2)), color=F.ORANGE,
                  label="shipped kernel")
    ...
    for x, style, align, dx in ((birth, "birth", "right", -0.04),
                                (death, "death", "left", 0.06)):
        curve_ax.axvline(x, lw=1.0, ls=(0, (2, 2)), color=F.FAINT, zorder=1)
        curve_ax.text(x + dx * x, 1.19, f"{style}$_B$ = {x:.2f}", ha=align,
                      va="center", fontsize=F.FS_SMALL, color=F.MUTED)

    # Every sample at its own d_B. The gap between the two clumps is the point.
    inside = d <= birth + 1e-9
    curve_ax.plot(d[inside], np.full(inside.sum(), -0.11), marker="|", ls="none",
                  ms=6, mew=0.9, color=F.BLUE, zorder=4)
    curve_ax.plot(d[~inside], np.full((~inside).sum(), -0.11), marker="|",
                  ls="none", ms=6, mew=0.9, color=F.FAINT, zorder=3)
    curve_ax.annotate("no sample lands in the\nshaded interval — on an\n"
                      "ultrametric a non-member\njoins at height $\\geq$ death$_B$",
                      xy=((birth + death) / 2, -0.11),
                      xytext=(death * 3.0, 0.34), ha="left", va="center",
                      fontsize=F.FS_SMALL, color=F.MUTED, linespacing=1.6,
                      arrowprops=dict(arrowstyle="->", lw=0.8, color=F.AXIS))

    F.style_axes(curve_ax,
    ...
    im_ = part_ax.imshow(U[:, order], aspect="auto", interpolation="nearest",
                         cmap=F.blue_cmap(reverse=False), vmin=0, vmax=1,
                         rasterized=True)
    part_ax.set_yticks(range(len(band.blocks)))
    part_ax.set_yticklabels([f"block {i + 1}" for i in range(len(band.blocks))],
                            fontsize=F.FS_SMALL)
    part_ax.set_xticks([])
    for s in part_ax.spines.values():
        s.set_color(F.AXIS)
        s.set_linewidth(0.8)
    part_ax.set_title(f"(b)  the band's fuzzy partition, {len(band.blocks)} blocks "
                      f"× {Dstar.shape[0]} points", fontsize=F.FS_LABEL,
                      color=F.INK, pad=6)
    part_ax.set_xlabel("samples, ordered by ground-truth fine label",
