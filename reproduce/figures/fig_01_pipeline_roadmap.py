...
def _claims():
    ...
    deepest = max(
        multiscale, key=lambda r: len(r["granularities recovered"].split(","))
    )
    ...
    def r2(model, refinement):
        row = next(
            r for r in concrete if r["Model"] == model and r["Refinement"] == refinement
        )
        return H.number(row["R²"])

    gain_0 = r2("flat MoG-TSK 0th", "refined") - r2(
        "flat MoG-TSK 0th", "closed-form only"
    )
    gain_2 = r2("flat MoG-TSK 2nd", "refined") - r2(
        "flat MoG-TSK 2nd", "closed-form only"
    )
...
def build():
    ...
    stages = [
        (
            F.FAINT,
            "Raw data",
            "feature table or dissimilarity matrix",
            None,
            False,
            None,
        ),
        (
            F.BLUE,
            "Structure discovery",
            "mergeVAT reorder · iVAT minimax transform",
            "Ch. 3",
            False,
            "structure",
        ),
        (
            ...
            "persistence-gated set cover over the hierarchy",
            "Ch. 5",
            False,
            "membership",
        ),
        (
            F.ORANGE,
            "FIS synthesis",
            "per-class Gaussian mixtures · ridge-TSK solve",
            "Ch. 4, 6",
            False,
            "synthesis",
        ),
        (
            F.VIOLET,
            "Refinement — optional",
            "L-BFGS-B polish of the antecedents",
            "Ch. 6, A",
            True,
            "refine",
        ),
        (
            F.FAINT,
            "Interpretable fuzzy model",
            "IF–THEN rules over named variables",
            None,
            False,
            None,
        ),
    ...
    for a, b, dashed in zip(ys, ys[1:], [False, False, False, True, True]):
        F.arrow(
            ...
            (X, a - HBOX / 2),
            (X, b + HBOX / 2),
            color=F.FAINT if dashed else F.AXIS,
    ...
    ax.text(
        50,
        2,
        "Refinement is dashed on purpose: the search everyone else puts on "
