...
def build():
    ...
    F.box(ax, 50, ROOT_Y, 30, 12, "Gate:  Cement",
          "fuzzy partition of unity\nover one named input",
          color=F.BLUE, title_size=F.FS_ANNOT, body_size=F.FS_SMALL - 0.5)

    mids = [(26, "Gate:  Age"), (74, "Gate:  Water")]
    for x, title in mids:
        F.box(ax, x, MID_Y, 30, 12, title,
              "fuzzy partition of unity\nover one named input",
              color=F.BLUE, title_size=F.FS_ANNOT, body_size=F.FS_SMALL - 0.5)

    leaves = [13, 39, 61, 87]
    for i, x in enumerate(leaves, start=1):
        F.box(ax, x, LEAF_Y, 22, 13, f"TSK expert {i}",
              "ridge least squares on\nthe points that reach it",
              color=F.ORANGE, title_size=F.FS_ANNOT, body_size=F.FS_SMALL - 0.5)
    ...
    edges = [((50, ROOT_Y - 6), (26, MID_Y + 6), "$\\mu_1$", (-3.5, 1)),
    ...
    for start, end, label, offset in edges:
        F.arrow(ax, start, end, color=F.AXIS, lw=1.2, label=label,
                label_offset=offset, label_size=F.FS_SMALL,
                label_color=F.INK_2)
    ...
    ax.text(50, 12, "Output = $\\sum_\\ell w_\\ell(x)\\, f_\\ell(x)$   with   "
                    "$w_\\ell(x) = \\prod_{\\mathrm{path}} \\mu$   —   every point "
                    "reaches every expert, to a degree",
            ha="center", va="center", fontsize=F.FS_ANNOT, color=F.INK_2)

    ax.text(50, 4, "Gates name only original inputs — no synthetic intermediate can "
