...
def _bar_panel(ax, values, spreads, title, ylabel, fmt):
    ...
    bars = ax.bar(
        x,
        values,
        color=[F.tint(c, 0.15) for c in COLOUR],
...
def build():
    ...
    _bar_panel(ax_mf, mf, mf_sd, "Membership-function count", "MF count", "{:.1f}")
    _bar_panel(
        ax_acc, acc, acc_sd, "Accuracy, and the paired Δ vs. base", "accuracy", "{:.3f}"
    )
