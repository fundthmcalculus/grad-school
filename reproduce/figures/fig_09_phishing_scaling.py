...
def _load(label=None):
    ...
    return (
        {
            route: {
                n: tuple(float(np.median([v[i] for v in vals])) for i in range(4))
                for n, vals in sizes.items()
            }
            for route, sizes in by.items()
        },
        src,
    )
...
def _cost(ax, by):
    ...
    series = [
        ("construction", F.BLUE, "-", 1.8, "construction (auto components)"),
    ...
    ax.plot(
        xs,
        [1000 * by["construction"][n][3] for n in xs],
        lw=1.4,
        ls=(0, (3, 2)),
        color=F.FAINT,
        zorder=3,
...
def _error(ax, by):
    ...
    for route, colour, label in (
        ("construction", F.BLUE, "construction"),
        (KM, F.ORANGE, "k-means"),
    ...
    F.style_axes(
        ax,
        title="(b)  error rate, not accuracy",
        xlabel="training rows (log)",
        ylabel="1 − test accuracy (log)",
    )
...
def _before_after(ax, by, capped, before):
    ...
    if before is not None:
        ...
        ax.plot(
            xs,
            [1000 * before["construction"][n][0] for n in xs],
            lw=1.5,
            ls=(0, (1, 2)),
            marker="v",
            ms=4.0,
    ...
    if capped is not None:
        ...
        ax.plot(
            xs,
            [1000 * capped["construction"][n][0] for n in xs],
            lw=1.5,
            ls=(0, (4, 2)),
            marker="s",
            ms=3.6,
    ...
    ax.plot(
        xs,
        [1000 * by["construction"][n][0] for n in xs],
        lw=1.8,
        marker="o",
        ms=4.0,
        color=F.BLUE,
    ...
    F.style_axes(
        ax,
        title="(c)  the two library fixes, priced",
        xlabel="training rows (log)",
        ylabel="milliseconds, single-threaded (log)",
    )
...
def _anatomy(ax, by, pinned):
    ...
    if pinned is None:
        ax.text(
            0.5,
            0.5,
            f"pinned companion run not found\n({PINNED})",
    ...
    x = np.arange(len(ns))
    ax.bar(x, fit, width=0.62, color=F.BLUE, label="mixture fit (k-means)", zorder=3)
    ax.bar(
        x,
        select,
        width=0.62,
    ...
    F.style_axes(
        ax,
        title="(d)  where the construction's time goes",
