...
distribution shapes.  This module prototypes two additional transforms, each
...
1. **QuantileUniformScaler** — wraps ``sklearn.preprocessing.QuantileTransformer``
   ...
   The gold standard for marginal uniformity, but fragile on small datasets.
...
2. **EmpiricalCDFScaler** — evaluates the empirical CDF of the training data
   ...
   quantiles.  Affine-maps to ``feature_range``.  Most robust performer across
   datasets in testing.

Both follow the sklearn transformer API (``fit`` / ``transform`` /
...
Usage::
    ...
    from uniformity_transforms import EmpiricalCDFScaler
    scaler = EmpiricalCDFScaler(feature_range=(0.0, 1.0))
