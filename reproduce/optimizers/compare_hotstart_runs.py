...
def main():
    ...
    for field, unit, scale in (("r2", " R²", 1.0), ("cv_mse", " CV MSE", 1.0)):
        d, n = _paired(
            b_seeds, a_seeds, ("arm", "init", "seed"), field, where=only_none_hot
        )
    ...
    b_i, a_i = _index(b_seeds, "arm", "init", "seed"), _index(
        a_seeds, "arm", "init", "seed"
    )
    arms = sorted({r["arm"] for r in a_seeds if cold(r)})
    for arm in arms:
        pairs = [
            (b_i[k], a_i[k]) for k in b_i if k in a_i and k[0] == arm and k[1] == "cold"
        ]
        if not pairs:
            continue
        ...
        print(
            f"   {arm:<15} before {bv.mean():7.0f} ± {bv.std():<6.0f}   "
    ...
    if worst:
        ...
        print(
            f"   widest pair: {x} - {y} = {d.mean():+.4f} ± {d.std():.4f} "
