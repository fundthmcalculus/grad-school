...
def main():
    ...
    print(
        f"phishing identification sweep: sizes={sizes}, components="
    ...
    for n in sizes:
        ...
        for seed in seeds:
            ...
            for name, build in routes:
                ...
                _write(records)
                print(
                    f"  n={len(Xtr):<7} {name:<18} acc={acc:.4f}  "
        ...
        if sel:
            fe_rows.append(
                [
                    f"{sel[0]['n_rows']:,}",
                    f"{sel[0]['n_features']}",
                    C.cell([1000 * r["screen_s"] for r in sel], fmt="{:.0f}"),
                ]
            )
    C.emit(
        "table_phishing_feature_engineering",
        "PhiUSIIL — feature engineering, shared by every route",
        ["train rows", "features kept", "screening (ms)"],
        fe_rows,
        note=(
            "`calculate_gaussian_correlation` + `take_top_features`, the "
    ...
    for name in dict.fromkeys(r["route"] for r in records):
        for n in sizes:
            sel = [
                r for r in records if r["route"] == name and r["n_rows"] == int(n * 0.8)
            ]
            if not sel:
                continue
            rows.append(
    ...
    C.emit(
        ...
        rows,
        note=(
            f"Binary classification, so the rule count is fixed at K = 2 for "
            ...
            + (
                ...
                if args.pin_components
                else f"chooses its own component count per (feature, class), so its "
...
def _archive(label, args, sizes, seeds):
    ...
    with open(os.path.join(dest, "PROVENANCE.txt"), "w") as f:
        f.write("\n".join(lines) + "\n")
    for name in (
        "table_phishing_identification.md",
