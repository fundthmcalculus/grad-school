...
def run_one(arm, seed, budget, start):
    ...
    return {
        "arm": arm, "seed": seed, "start": start, "structure": found,
        "cv_mse": obj.best_f, "cv_mse_0": obj.f0, "r2": r2, "rmse": rmse,
        "n_rules": n_rules, "n_mfs": n_mfs,
        "ref_r2": ref_r2, "ref_rmse": ref_rmse, "ref_rules": ref_rules,
        "ref_mfs": ref_mfs, "evals": obj.n_evals, "error": error,
    }
...
def main():
    ...
    print(f"structure search: budget={args.budget} model builds/arm/seed, "
    ...
    C.emit(
        ...
        "(concrete)",
        ["arm", "rules found (modal)", "order (modal)", "features (modal)",
         "membership fns", "test R²", "Δ R² vs shipped default", "wins"],
        rows,
        note=(f"Five decision variables: output buckets (= rules), Gaussians per "
              ...
              f"not justified." if ref else ""))

    path = os.path.join(C.OUTPUT_DIR, "table_opt_structure_seeds.csv")
    C.write_csv(path, ["arm", "seed", "start", "n_buckets", "n_gaussians",
                       "top_n", "order", "log10_l2", "n_rules", "n_mfs",
                       "cv_mse", "r2", "rmse", "ref_r2", "ref_rmse", "evals"],
                [[r["arm"], r["seed"], r["start"], r["structure"]["n_buckets"],
                  r["structure"]["n_gaussians"], r["structure"]["top_n"],
                  r["structure"]["order"], f"{r['structure']['log10_l2']:.4f}",
                  r["n_rules"], r["n_mfs"], f"{r['cv_mse']:.6f}",
                  f"{r['r2']:.6f}", f"{r['rmse']:.4f}", f"{r['ref_r2']:.6f}",
                  f"{r['ref_rmse']:.4f}", r["evals"]] for r in records])
    print(f"  wrote {path}")
    return 0
