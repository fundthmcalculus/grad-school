...
def write_markdown(path, title, header, rows, note="", seeds=None):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        # `gated-minimax-selection/outputs/results.json`, which its driver produced at
        # `SEEDS = [0, 1, 2, 3, 4]`, with `NERFCM_SEEDS = [0, 1, 2]` and fixed
        # per-generator seeds for the multi-scale and selection experiments. So all
        # three Chapter 5 tables carried a footer claiming ten seeds over a five-seed
        # run. That is the `PROVENANCE.txt` seed-count bug again -- a file stating a
        # seed list it did not use -- one directory over, and it is worse here because
        # the footer is what gets quoted into a chapter.
