#!/usr/bin/env python
"""Quick single-example baseline on Shuttle dataset."""
...
print("=" * 70)
print("Shuttle Clustering — Single Example Baseline")
print("=" * 70)
print()
...
X, y = F.load_shuttle()
if X is None:
    print("ERROR: Could not load Shuttle")
    sys.exit(1)

print(f"Dataset: {len(X)} rows × {X.shape[1]} features, {len(np.unique(y))} classes")
classes = np.unique(y)
print(f"Classes: {classes}")
class_dist = [(c, (y == c).sum()) for c in sorted(classes)]
...
print("Summary")
print("=" * 70)
print(f"Dataset: Shuttle, {len(X):,} samples, 7 classes")
print(f"Split: 80/20 (train={len(Xtr):,}, test={len(Xte):,})")
print()
print(f"Random Forest (200 trees):")
print(f"  Training: {rf_train_time:.3f}s")
print(f"  Prediction: {rf_pred_time:.4f}s (on {len(Xte):,} samples)")
print(f"  Accuracy: {rf_acc:.4f}")
print()
try:
    print(f"MoG Classifier:")
    print(f"  Training: {mog_train_time:.3f}s")
    print(f"  Prediction: {mog_pred_time:.4f}s (on {len(Xte):,} samples)")
    print(f"  Accuracy: {mog_acc:.4f}")
    print()
...
# Baseline 3: MoG without feature ranking (all features)
print()
print("Baseline 3: MoG Classifier (no feature ranking, all features)")
try:
    from tribblefis.gaussian_classifier import TribbleClassifier

    mog_no_rank = TribbleClassifier(top_n=-1, random_state=42)
    t0 = time.time()
    mog_no_rank.fit(Xtr, ytr)
    mog_nr_train_time = time.time() - t0
    t0 = time.time()
    mog_nr_pred = mog_no_rank.predict(Xte)
    mog_nr_pred_time = time.time() - t0
    mog_nr_acc = accuracy_score(yte, mog_nr_pred)
    print(f"  Training: {mog_nr_train_time:.3f}s")
