...
LAMBDAS = [
    float(x)
    for x in os.environ.get("REPRO_LAMBDAS", "0.01,0.5,1.0,1.5,2.0,2.5").split(",")
]
...
def evaluate(X, y_raw, seed, scheme, n=BUCKETS, order=ORDER):
    ...
    corr, cent2 = solve_tsk_consequents(
        Xtr,
        memb,
        top,
        cent,
    ...
    pred = np.asarray(
        predict_tsk(
            Xte, memb, top, cent2, corr, order=order, basis="raw", cross_pairs=None
        ),
        float,
    ).ravel()
...
def main():
    ...
    print(
        f"  synthetic: n={N_SYNTH} m={M_SYNTH} buckets={BUCKETS} order={ORDER} "
        f"seeds={C.SEEDS}"
    )
    ...
    for lam in LAMBDAS:
        ...
        for seed in C.SEEDS:
            ...
            for scheme in ("uniform", "quantile"):
                try:
                    ...
                    print(
                        f"    [{scheme} lam={lam}] seed {seed}: {exc.__class__.__name__}"
                    )
                    continue
                if r:
        ...
        if um is None or qm is None:
            continue
        print(
            f"  {lam:7.2f} {sk:+7.2f} {um:11.3f} {qm:12.3f} {qm-um:+8.3f} "
    ...
    C.emit(
        ...
        [
            ...
            "uniform tail RMSE",
            "quantile tail RMSE",
            "uniform min bucket n",
        ],
        rows,
        note=(
            f"A fixed linear signal is pushed through y = expm1(λz)/λ, a strictly "
