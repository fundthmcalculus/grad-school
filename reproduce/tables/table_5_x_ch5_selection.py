...
def table_5_1(results):
    ...
    header = ["Dataset", "single-linkage on D", "NERFCM on raw D",
              "NERFCM on D* (given k)", "ConiVAT (constrained)",
              "set-cover, covered pts", "set-cover, all pts (gated)",
              "k discovered", "coverage"]
    rows = []
    ...
    C.emit(
        "table_5_1_battery", "Table 5.1 -- The battery (adjusted Rand index)",
        header, rows,
        note=("Baseline columns are `main_table.{iVAT_SL_ari, NERFCM_D_ari, "
...
def table_5_2(results):
    ...
    header = ["Dataset", "levels", "flat cover (mean ARI)", "multi-scale (mean ARI)",
              "granularities recovered", "flat k"]
    rows = []
    ...
    C.emit(
        "table_5_2_multiscale", "Table 5.2 -- Multi-scale recovery (ARI, averaged over all ground-truth levels)",
        header, rows,
        note=("From `multiscale_hierarchy.*` in results.json: flat_mean_ari, ms_mean_ari, "
...
def table_5_3(results):
    ...
    header = ["Selection method", "bridge case (ARI)", "bridge k", "bridge coverage",
              "noise k (true: none)", "noise coverage"]
    rows = []
    ...
    C.emit(
        "table_5_3_selection", "Table 5.3 -- Selection-method comparison",
        header, rows,
        note=("From `persistence_methods.{bridged_gaussians, uniform_noise}.methods.*`. "
