...
print(f"correctness ED slice (200x{sub.shape[1]}): max|diff|={diff.max():.3e}  allclose={np.allclose(D_aeon, D_ours)}")

lng = np.ascontiguousarray(rng.normal(size=(40, 1024)))
Da = dtw_pairwise_distance(lng)
Do = dtw_simd.dtw_pairwise(lng)
print(f"correctness long (40x1024): max|diff|={np.abs(Da-Do).max():.3e}  allclose={np.allclose(Da, Do)}")

# --- throughput --------------------------------------------------------------
def cells(n, L):
    return (n * (n - 1) / 2) * L * L

for label, data in (("ED-like 600x96", X[:600]), ("SLC-like 150x1024", np.ascontiguousarray(rng.normal(size=(150, 1024))))):
    n, L = data.shape
    t0 = time.time(); dtw_pairwise_distance(data); t_a = time.time() - t0
    t0 = time.time(); dtw_simd.dtw_pairwise(data); t_o = time.time() - t0
    ra, ro = cells(n, L) / t_a, cells(n, L) / t_o
    print(f"{label}: aeon {t_a:.1f}s ({ra:.2e} cells/s) | simd {t_o:.2f}s ({ro:.2e} cells/s) | speedup x{t_a/t_o:.1f}")
...
print(f"  StarLightCurves FULL N=9236: {t_slc_full:.1f} h  (aeon measured-rate estimate was ~30 h)")
