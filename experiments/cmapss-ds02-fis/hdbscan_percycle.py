...
def cluster(tab, feat_cols):
    ...
    return dict(
        emb=emb, labels=labels, rul=rul,
        unit=tab["unit"].to_numpy(), n_pca=Xr.shape[1], k=k,
...
print(f"  {len(raw_tab)} engine-cycles, {len(cols)} features, "
...
for name, tab in (("raw sensors", raw_tab), ("condition-corrected", cor_tab)):
    ...
    r = cluster(tab, cols)
    results[name] = r
    print(f"  PCA kept {r['n_pca']} comps ({VAR_KEEP:.0%} var); "
...
def scat(ax, xy, c, cat=False, cmap=None, cbar=None, legend=False):
    if cat:
        ...
        sm = ax.scatter(xy[:, 0], xy[:, 1], s=10, c=c, cmap=cmap, alpha=0.85,
                        linewidths=0)
        if cbar:
...
for row, name in enumerate(("raw sensors", "condition-corrected")):
    ...
    axes[row, 2].set_title(
        ...
        f"AMI(health)={r['ami_health']:.2f}", fontsize=10)
