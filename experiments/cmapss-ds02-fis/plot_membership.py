...
fig.suptitle("DS02: Gaussian vs fast-histogram trapezoid membership functions "
...
for ax, (key, ylab) in zip(axes, metrics):
    ...
    ax.set_xticks(x); ax.set_xticklabels(viable); ax.set_ylabel(ylab)
    ax.grid(axis="y", alpha=0.3)
    vals = df[df.mf.isin(viable)][key]
    ax.set_ylim(vals.min() - 0.5, vals.max() + 0.6)
    if key == "per_sample":
        ax.axhline(6.48, ls="--", color="0.5", lw=1); ax.legend(fontsize=8, loc="upper left")
