...
os.makedirs(OUT, exist_ok=True); CSV = f"{OUT}/mf_ds02.csv"
CONFIGS = [("full-2nd (top_p=0.95)", dict(tsk_order="full-2nd", top_p=0.95)),
           ("2nd (top_p=0.99)", dict(tsk_order="2nd", top_p=0.99))]
MFS = [("gaussian",   dict(member_function="gaussian")),
...
for cname, ckw in CONFIGS:
    for mfname, mfkw in MFS:
        t = time.perf_counter()
        eng = TribblePredictiveHealth(condition_correction=False, aggregation="raw_memory",
                                      l2_reg=0.01, **ckw, **mfkw)
        eng.fit_featurized(train_tab, feat_cols)
        m = eng.score_featurized(test_tab); dt = time.perf_counter() - t
        r = dict(config=cname, mf=mfname, per_sample=m["per_sample_rmse"],
                 monotone=m["monotone_cycle_rmse"], per_engine=m["per_engine_rmse"],
                 n_rules=eng.n_rules_, fit_s=dt)
        rows.append(r)
        open(CSV, "a").write(f"{cname},{mfname},{r['per_sample']:.4f},{r['monotone']:.4f},"
                             f"{r['per_engine']:.4f},{r['n_rules']},{dt:.1f}\n")
        print(f"  {cname:22s} {mfname:11s} per-sample={r['per_sample']:.3f} "
              f"monotone={r['monotone']:.3f} per-engine={r['per_engine']:.3f} ({dt:.1f}s)", flush=True)

df = pd.DataFrame(rows)
plt.rcParams.update({"font.size": 9, "figure.dpi": 130})
fig, axes = plt.subplots(1, 2, figsize=(13, 5.0))
fig.suptitle("DS02: antecedent membership-function shape (Gaussian vs trapezoid vs triangular)\n"
             "raw_memory, l2_reg=0.01", fontsize=12)
mf_order = [m[0] for m in MFS]; x = np.arange(len(mf_order)); w = 0.35
colors = {"full-2nd (top_p=0.95)": "#e8590c", "2nd (top_p=0.99)": "#2f9e44"}
for ax, key, ylab in [(axes[0], "per_sample", "per-sample RMSE"),
    ...
    ax.set_xticks(x); ax.set_xticklabels(mf_order); ax.set_ylabel(ylab)
    ax.grid(axis="y", alpha=0.3); ax.legend(fontsize=8)
    lo = df[key].min(); ax.set_ylim(lo - 0.4, df[key].max() + 0.3)
axes[0].axhline(6.48, ls="--", color="0.5", lw=1)
axes[0].text(len(mf_order)-1, 6.48, " gaussian full-2nd best 6.48", color="0.4",
