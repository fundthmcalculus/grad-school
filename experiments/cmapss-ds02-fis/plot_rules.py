...
fig.suptitle("DS02: adding rules (output buckets) vs consequent order  "
             "[top_p=0.99, l2_reg=0.01]\n"
             "extra simple rules substitute for full-2nd cross terms — most "
             "dramatically on the canonical per-engine endpoint", fontsize=12)
panels = [("per_sample", "per-sample RMSE (all rows)"),
...
for ax, (key, ylab) in zip(axes, panels):
    ...
    if key == "per_sample":
        ...
        ax.text(df.n_rules.max(), FULL2ND_095, " full-2nd best 6.48", color="#c0392b",
                fontsize=8, va="bottom", ha="right")
        ax.text(df.n_rules.max(), CNN, " CNN 7.22", color="0.4", fontsize=8,
                va="bottom", ha="right")
    ax.set_xlabel("number of rules (= output buckets)")
    ax.set_ylabel(ylab); ax.grid(alpha=0.3); ax.legend(fontsize=8)
