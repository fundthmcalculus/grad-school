...
def per_cycle(eng, table):
    ...
    return (s.groupby(["engine", "cycle"])
            .agg(pred=("predicted_rul", "mean"), true=("rul", "mean")).reset_index())
...
def main(h5_dir):
    ...
    print(f"  train engines {tr_rm['engine'].nunique()}  -> "
    ...
    base_cfg = {k: v for k, v in cad.CONFIGS["raw_memory"].items() if k != "n_output_buckets"}
    A = []
    for nb in (2, 3, 4, 6, 8):
        cfg = {**base_cfg, "n_output_buckets": nb}
        val_ps = fit_rm(cfg, fit_rm_t, cols_rm).score_featurized(val_rm_t)["per_sample_rmse"]
        m = fit_rm(cfg, tr_rm, cols_rm).score_featurized(te_rm)
        print(f"  {nb:2d} | {val_ps:13.2f} | {m['per_sample_rmse']:14.2f} | {m['per_engine_rmse']:14.2f}")
    ...
    print(f"  -> validation picks nb={nb_val}. TEST per-sample: nb=2 {nb2[2]:.2f} vs "
    ...
    rm_fit, wc_fit = fit_rm(cad.CONFIGS["raw_memory"], fit_rm_t, cols_rm), \
        fit_rm(cad.CONFIGS["whole_cycle"], fit_wc_t, cols_wc)
    rm_full, wc_full = fit_rm(cad.CONFIGS["raw_memory"], tr_rm, cols_rm), \
        fit_rm(cad.CONFIGS["whole_cycle"], tr_wc, cols_wc)
    ...
    print("  alpha | VAL per-engine | TEST per-engine | TEST NASA   (alpha=1 -> whole_cycle)")
    B = []
    ...
    print(f"  -> validation picks alpha={a_val}. TEST per-engine: whole_cycle(a=1) "
