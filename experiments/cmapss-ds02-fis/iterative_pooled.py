...
def prep(h5_dir):
    ...
    print(f"  pooled {len(processed)} datasets, train {len(train):,} test {len(test):,}")

    caps = onset_caps(train, unit_col="engine")            # caps on the full train
    sub = train.sample(MAX_TRAIN, random_state=SEED) if len(train) > MAX_TRAIN else train
...
def boost(X_tr, y_tr, X_te, y_te, cfg, n_stages, eta):
    F_tr = np.zeros_like(y_tr); F_te = np.zeros(len(y_te))
    curve = []
...
def main(h5_dir):
    ...
    for eta in (1.0, 0.5, 0.3):
        ...
        print(f"  eta={eta}: train {curve[0][0]:.2f}->{curve[-1][0]:.2f} (min {curve[i][0]:.2f}@{i+1})  "
    ...
    for nb in (2, 3, 4, 6, 8):  # 12 buckets OOMs the 128k-row full-2nd predict on a 15GB box
        r = _fit({**BASE, "n_output_buckets": nb}, X_tr, y_tr, seed=SEED)
        tr, te = rmse(y_tr, r.predict(X_tr)), rmse(y_te, r.predict(X_te))
        print(f"  buckets={nb:2d} rules={int(r.model_.n_rules):2d}: train {tr:.3f}  test {te:.3f}")
        rows.append((f"rules buckets={nb}", nb, tr, te))

    csv_path = os.path.join(OUT, "iterative_pooled.csv")
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f); w.writerow(["method", "param", "train_rmse", "test_rmse"])
        w.writerows(rows)
    print(f"\nwrote {csv_path}")
    _plot(rows)
...
def _plot(rows):
    ...
    for name, pts in boosts.items():
        pts.sort()
        axL.plot([p[0] for p in pts], [p[1] for p in pts], "-o", ms=3, label=f"{name} train")
        axL.plot([p[0] for p in pts], [p[2] for p in pts], "--", alpha=.6, label=f"{name} test")
    axL.set_title("residual boosting"); axL.set_xlabel("boosting stage")
    rc = sorted([(r[1], r[2], r[3]) for r in rows if r[0].startswith("rules")])
    axR.plot([p[0] for p in rc], [p[1] for p in rc], "-o", ms=3, c="tab:green", label="train")
    axR.plot([p[0] for p in rc], [p[2] for p in rc], "--", c="tab:green", alpha=.6, label="test")
    axR.set_title("staged rule growth"); axR.set_xlabel("output buckets (rules)")
    axL.set_ylabel("RMSE (cycles)")
    for ax in (axL, axR):
        ax.grid(alpha=.3); ax.legend(fontsize=7)
    fig.suptitle("Pooled all-datasets: iterative reduction of the training residual", y=1.02)
    fig.tight_layout()
    p = os.path.join(OUT, "iterative_pooled.png"); fig.savefig(p, dpi=130, bbox_inches="tight")
    print(f"wrote {p}")
