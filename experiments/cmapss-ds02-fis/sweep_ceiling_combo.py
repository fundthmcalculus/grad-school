...
def main():
    ...
    combos = [
        ...
        (
            "stride100+ceil60",
            dict(stride=100, window=5, memory=2, ceiling=60, extra=None),
        ),
        ("win10+ceil60", dict(stride=200, window=10, memory=2, ceiling=60, extra=None)),
        (
            "ng2+ceil60",
            dict(stride=200, window=5, memory=2, ceiling=60, extra={"n_gaussians": 2}),
        ),
        (
            "ALL(s100/w10/ng2/c60)",
            dict(stride=100, window=10, memory=2, ceiling=60, extra={"n_gaussians": 2}),
        ),
    ]
    for tag, c in combos:
        X_tr, y_tr, X_te, y_te = prep(
            dev, test, sensors, c["stride"], c["window"], c["memory"], c["ceiling"]
        )
