...
OUT = "outputs/hdbscan-ds02"; os.makedirs(OUT, exist_ok=True)
...
for order in ORDERS:
    for nb in BUCKETS:
        ...
        eng = TribblePredictiveHealth(condition_correction=False, aggregation="raw_memory",
                                      tsk_order=order, top_p=0.99, l2_reg=0.01,
                                      n_output_buckets=nb)
        eng.fit_featurized(train_tab, feat_cols)
        m = eng.score_featurized(test_tab)
        dt = time.perf_counter() - t
        r = dict(order=order, buckets=nb, n_rules=eng.n_rules_,
                 ...
                 per_engine=m["per_engine_rmse"], fit_s=dt)
        rows.append(r)
        with open(CSV, "a") as f:
            f.write(f"{order},{nb},{r['n_rules']},{r['p']},{r['n_params']},"
                    f"{r['per_sample']:.4f},{r['monotone']:.4f},{r['per_engine']:.4f},{dt:.1f}\n")
        print(f"  {order:9s} buckets={nb:2d} rules={r['n_rules']:2d} "
              f"params={r['n_params']:5d} per-sample={r['per_sample']:.3f} "
              f"monotone={r['monotone']:.3f} ({dt:.1f}s)", flush=True)

df = pd.DataFrame(rows)
for order in ORDERS:
    sub = df[df.order == order]; b = sub.loc[sub.per_sample.idxmin()]
    print(f"\n=== {order} === best per-sample {b.per_sample:.3f} @ {int(b.n_rules)} rules "
          f"({int(b.n_params)} params, monotone {b.monotone:.2f})", flush=True)

plt.rcParams.update({"font.size": 9, "figure.dpi": 130})
fig, axes = plt.subplots(1, 2, figsize=(13, 5.2))
fig.suptitle("DS02: more rules vs consequent order (top_p=0.99, l2_reg=0.01)\n"
             "can extra rules substitute for full-2nd cross terms?", fontsize=12)
...
for ax, key, ylab in [(axes[0],"per_sample","per-sample RMSE"),
    ...
    ax.set_xlabel("number of rules (= output buckets)"); ax.set_ylabel(ylab)
    ax.grid(alpha=0.3); ax.legend(fontsize=8)
axes[0].text(df.n_rules.max(), FULL2ND, " full-2nd@2rules 6.48", color="#c0392b",
             fontsize=8, va="bottom", ha="right")
axes[0].text(df.n_rules.max(), CNN, " CNN 7.22", color="0.4", fontsize=8, va="bottom", ha="right")
