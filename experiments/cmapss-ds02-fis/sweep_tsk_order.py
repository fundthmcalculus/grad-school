...
for label, kw in CONFIGS:
    ...
    per_engine = {
        u: rmse(frame[frame.unit == u]["true"], frame[frame.unit == u]["rul"])
        for u in TEST_UNITS
    }
    rows.append(
        dict(
    ...
    print(
        f"  {label:16s}: p={p}  terms/rule={terms_per_rule:4d}  "
...
hdr = (
    f"{'order':9s} {'terms/rule':>10s} {'params':>7s} {'params vs full':>14s} "
...
for r in rows:
    print(
        f"{r['order']:9s} {r['terms_per_rule']:10d} {r['n_params']:7d} "
...
ax.plot(
    [r["n_params"] for r in frontier],
    [r["per_sample"] for r in frontier],
    "-o",
    color="#2b6cb0",
    markersize=9,
    zorder=2,
...
ax.set_title(
    "DS02 raw_memory: consequent order vs accuracy\n"
    "the full-2nd cross terms earn their complexity — and don't prune",
    fontsize=11,
)
ax.grid(alpha=0.3)
fig.tight_layout()
