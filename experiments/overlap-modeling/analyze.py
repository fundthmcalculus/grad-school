...
def headline(df) -> str:
    ...
    if not sr.empty and not sa.empty:
        ...
        pick_sr = (
            sr.loc[sr.groupby(CELL)["r2_val"].idxmax()]
            .set_index(CELL)[["r2_test"]]
            .rename(columns={"r2_test": "r2_test_rand"})
        )
...
def by_buckets(df, fractions) -> str:
    ...
    lines = [
        ...
        "",
        "| family | dataset | "
        + " | ".join(f"{b} buckets" for b in sorted(df.n_buckets.unique()))
        + " |",
...
def fusion(df, fusion_regs) -> str:
    lines = [
        ...
        "",
        "| dataset | baseline | "
        + " | ".join(f"λ={lam:g}" for lam in fusion_regs)
        + " |",
        "|---|---|" + "---|" * len(fusion_regs),
    ]
    for dataset, part in df.groupby("dataset"):
...
def mechanism(df, fractions) -> str:
    ...
    lines = [
        ...
        "",
        "| dataset | τ=0 (baseline) | "
        + " | ".join(f"τ={f:g}" for f in fractions)
        + " |",
        "|---|---|" + "---|" * len(fractions),
    ]
    for dataset, part in df.groupby("dataset"):
...
def figure(df, fractions, path) -> str | None:
    ...
    fig, axes = plt.subplots(
        1, len(datasets), figsize=(4.1 * len(datasets), 3.6), sharey=False
    )
    axes = np.atleast_1d(axes)
    colors = {
        "soft-ante": "#1f77b4",
        "soft-random": "#2ca02c",
        "local-overlap": "#d62728",
        "full-overlap": "#9467bd",
    }
    ...
    fig.suptitle(
        "Test R² against overlap width — diagnostic, not a selection table", fontsize=10
    )
...
def main():
    ...
    header = (
        f"<!-- generated by analyze.py from {os.path.basename(args.results)}; "
    ...
    print(
        f"wrote outputs/{os.path.basename(drawn)}"
        if drawn
        else "matplotlib unavailable; figure skipped"
    )

    dropped = (
        df[["dropped_val", "dropped_test"]].to_numpy().sum()
        if "dropped_test" in df
        else 0
    )
    print(
    ...
    if len(errors):
        print(
            errors[["dataset", "n_buckets", "order", "label", "error"]]
            .head(10)
            .to_string()
        )
    print("\n" + written["headline.md"])
