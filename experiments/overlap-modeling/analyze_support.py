...
def support_table(df) -> str:
    ...
    lines = ["## Compact support: locality, coverage, accuracy", "",
...
def locality_gap(df) -> str:
    ...
    lines = ["## Does the local fit close on the global one as support tightens?", "",
    ...
    for shape in _shape_order(df):
        ...
        lines.append(f"| {shape} | {_mean(sub.active_frac)} | {_mean(sub.uncovered)} | "
                     f"{_pm(gap)} |")
    return "\n".join(lines) + "\n"
...
def figure(df, path) -> str | None:
    ...
    ks = sorted({float(s.split("/")[1]) for s in df["shape"].unique()
                 if s.startswith("clamped-smooth/")})
    datasets = sorted(df.dataset.unique())
    fig, axes = plt.subplots(2, len(datasets), figsize=(3.6 * len(datasets), 6.0),
                             sharex=True)
    ...
    for col, dataset in enumerate(datasets):
        ...
        def series(field, fit):
            return [part[(part["shape"] == f"clamped-smooth/{k:g}")
                         & (part.fit == fit)][field].mean() for k in ks]

        ax = axes[0, col]
        ax.plot(ks, series("r2_test", "global"), "-o", ms=3.5, color="#1f77b4",
                label="global solve")
        ax.plot(ks, series("r2_test", "local"), "-s", ms=3.5, color="#d62728",
                label="per-bucket solve")
        ax.axhline(gauss[gauss.fit == "global"].r2_test.mean(), ls="-", color="k",
                   lw=1.3, label="gaussian, global")
        ax.axhline(gauss[gauss.fit == "local"].r2_test.mean(), ls=":", color="#d62728",
                   lw=1.3, label="gaussian, per-bucket")
        ...
        ax2 = axes[1, col]
        ax2.plot(ks, series("active_frac", "global"), "-o", ms=3.5, color="#2ca02c",
                 label="active_frac (locality)")
        ax2.plot(ks, series("uncovered", "global"), "-^", ms=3.5, color="#ff7f0e",
    ...
    fig.legend(h1 + h2, l1 + l2, loc="lower center", ncol=6, fontsize=7.5,
               frameon=False, bbox_to_anchor=(0.5, -0.04))
    fig.suptitle("Clamping a Gaussian: accuracy against the locality/coverage trade-off",
                 fontsize=10)
...
def main():
    ...
    ap.add_argument("--results",
                    default=os.path.join(HERE, "outputs", "support_results.json"))
    args = ap.parse_args()
    ...
    header = (f"<!-- generated by analyze_support.py from "
