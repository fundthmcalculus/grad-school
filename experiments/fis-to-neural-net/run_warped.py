...
Writes `outputs/warped_results.json` and `outputs/warped.md`. Leaves `run_simplicial.py`'s
outputs untouched.
"""
...
def main() -> int:
    ...
    ap.add_argument("--out", default=os.path.join(OUTPUTS, "warped_results.json"))
    ap.add_argument(
    ...
    path = os.path.join(OUTPUTS, "warped.md")
