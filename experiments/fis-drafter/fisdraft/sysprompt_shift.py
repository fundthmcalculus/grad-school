...
def run(seeds=6, n_pca=32, score="trimmed") -> dict:
    ...
    for ai, A in enumerate(PROMPTS):
        ...
        for seed in range(seeds):
            ...
            benA = np.where(yA == 0)[0]; rng.shuffle(benA)
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                det = TribbleOneClassDetector(
                    ...
                    cov="pca", n_gaussians=1, score=score, random_state=seed).fit(XAdf.iloc[fit])
            ...
            for bi, B in enumerate(PROMPTS):
                ...
                benB = np.where(yB == 0)[0]; injB = np.where(yB == 1)[0]
    ...
    fpr_mat /= seeds; det_mat /= seeds
    return {"prompts": PROMPTS, "score": score,
...
def layer_features_transform(act_fit_src, fit_idx, act_target):
    ...
    for l in range(Lp1):
        ...
        p = PCA(n_components=min(8, D, len(fit_idx) - 1),
                random_state=0).fit(act_fit_src[fit_idx, l, :])
...
def main():
    ...
    print("REALIZED FPR (threshold set at 1% on train-A benign; rows=train A, cols=test B):")
    print("        " + "".join("%9s" % b for b in P))
    for i, A in enumerate(P):
        print("%-8s" % A + "".join("%9.3f" % r["realized_fpr"][i][j] for j in range(len(P))))
    print("\nDET@1%FP (oracle, rows=train A, cols=test B):")
    print("        " + "".join("%9s" % b for b in P))
    for i, A in enumerate(P):
        print("%-8s" % A + "".join("%9.3f" % r["det_at_1fp"][i][j] for j in range(len(P))))
