...
def run(rundir: Path, variant="mean", seed=0, n_vals=(10, 25, 50)) -> dict:
    ...
    out: dict = {
        "model": json.loads((rundir / "meta.json").read_text())["config"]["model_id"],
    ...
    for k in (1, 2, 3, 5):
        ...
        out["unsupervised"][f"gmm_k={k}"] = {
            ...
            "test_within_len": round(
                auc_within_len(yt, st, np.r_[tl[ben_test], tl[inj]]), 3
            ),
...
def main():
    ...
    for k, v in base["unsupervised"].items():
        print(
            f"  {k:10s} auroc {v['test_auroc']:.3f}  within-len {v['test_within_len']:.3f}"
