...
def main() -> None:
    ...
    def ray_proxy(ctrl) -> float:
        return max(ray_radius_proxy(ctrl, plant, p, dirs, RADIUS_CAP)
                   for p in candidates(ctrl) if p is not None)
    ...
    def sos_per_candidate(ctrl) -> dict[str, float]:
        ...
        for name, p in zip(("Riccati", "lin"), candidates(ctrl)):
            if p is None:
                continue
            rho = max_certified_rho(rat, a_mat, b_vec, p, lo=1e-3, hi=1e3,
                                    iters=14)
            if rho <= 0:
                continue
            rho *= 0.95
    ...
    for seed in (0, 1, 2):
        cl = proxy_ball_radius(warm, plant, p_riccati,
                               ball_samples(seed=seed), RADIUS_CAP)
        ry = ray_radius_proxy(warm, plant, p_riccati,
                              sphere_directions(N_DIRS, seed=seed), RADIUS_CAP)
    ...
    print(f"   {'controller':>14} {'score':>8} {'ray':>8} {'SOS':>8} "
    ...
    def perturbed(mag: float) -> TskController:
        ...
        v /= np.linalg.norm(v)
        return TskController(warm.centers, warm.widths,
    ...
    for tag, ctrl in [("imitation", warm)] + [
        ...
        if sb > 0:
            cal.append(sb / ry)
            print(f"   {tag:>14} {sc:>8} {ry:>8.4f} {sb:>8.4f} {nm:>8} "
                  f"{sb / ry:>8.4f}")
        else:
            print(f"   {tag:>14} {sc:>8} {ry:>8.4f} {'none':>8} {'--':>8} "
                  f"{'--':>8}")

    kappa = float(np.median(cal))
    print(f"   kappa = median(SOS/ray) = {kappa:.4f}  "
    ...
    print(f"   Penalty hinges on the CALIBRATED estimate kappa*ray, at w="
    ...
    print("   the other.")
    print(f"   {'spec':>6} {'pass':>5} {'score':>8} {'holdout':>8} "
    ...
    for spec in SPECS:
        ...
        for attempt in (1, 2, 3):
            ...
            t = time.time()
            opt, _, _ = policy_optimize(plant, warm, Z0S, ref, maxfev=MAXFEV,
            ...
            print(f"   {spec:>6.2f} {attempt:>5} {sc:>8} {sh:>8} {rc:>10.4f} "
