...
def main() -> None:
    ...
    print(
        f"{'lam':>8} {'||e||_L2':>12} {'||de||_L2':>12} {'action':>14} {'dof searched':>13}"
    )
    fits = {}
    for lam in (0.0, 0.25, 1.0, 4.0, 16.0):
        ...
        fits[lam] = f
        print(
            f"{lam:8.2f} {f.l2_error:12.4f} {f.h1_seminorm_error:12.4f} "
    ...
    for lam in (0.25, 1.0, 4.0, 16.0):
        f = fits[lam]
        print(
            f"  lam={lam:<6} slope error {f.h1_seminorm_error:8.4f} "
    ...
    print(
        "  on a non-convex antecedent landscape, so individual rows land in different"
    )
    ...
    print(
        f"{'MF kind':>9} {'width':>7} {'coherence':>11} {'off-block':>11} "
        f"{'min cover':>10} {'joint S':>13} {'greedy gap':>11}"
    )
    cases = [("gaussian", w) for w in (7.0, 5.0, 3.0, 2.0)] + [
        ("bump", 5.0),
        ("bump", 6.0),
    ]
    for kind, width in cases:
        ...
        _, joint, _ = solve_consequents(
            centers, widths, yd, dyd, quad, lam, 1, kind, X_REF
        )
        _, greedy = sequential_fit(centers, widths, yd, dyd, quad, lam, 1, kind, X_REF)
        print(
            f"{kind:>9} {width:7.1f} {rep.coherence:11.3e} {rep.off_block_energy:11.3e} "
            f"{rep.min_coverage:10.3e} {joint:13.2f} {abs(greedy - joint) / joint:11.3e}"
        )
    print(
        "  bump/5.0 tiles [-15,15] with exactly touching supports: off-block energy is 0"
    )
    print(
        "  and greedy == joint to machine precision -- but min coverage collapses to 0 at"
    )
    print(
        "  the seams and the action is 42x worse than the Gaussian of the same width."
    )
    print(
        "  Widening to bump/6.0 restores overlap and the action, and immediately brings"
    )
    ...
    for width in (7.0, 5.0, 3.0, 2.0):
        widths = np.full(3, width)
        _, joint, _ = solve_consequents(
            centers, widths, yd, dyd, quad, lam, 1, "gaussian", X_REF
        )
        _, greedy = sequential_fit(
            centers, widths, yd, dyd, quad, lam, 1, "gaussian", X_REF
        )
        ...
        print(
            f"  width={width:<5} joint={joint:11.4f}  naive greedy gap="
            f"{abs(greedy - joint) / joint:9.2e}  OLS greedy gap="
            f"{abs(ols - joint) / joint:9.2e}"
        )

    rule(
        "E. Identifiability constraints decide whether local optimality is certifiable"
    )
    span = X_HI - X_LO
    pitch = span / 3
    configs = [
    ...
    for label, gap, wb in configs:
        ...
        print(
            f"    Gram cond      {cert['gram_condition']:.3e}  "
            f"(min eig {cert['gram_min_eigenvalue']:.3e})"
        )
        print(
            f"    Galerkin resid {cert['galerkin_residual_inf_norm']:.3e}  "
            f"(consequent block)"
        )
        print(
            f"    reduced |grad| {cert['reduced_gradient_norm']:.3e}  (antecedent block)"
        )
        print(
            f"    KKT residual   {cert['kkt_residual']:.3e}  "
            f"(gradient projected onto the critical cone -> "
            f"stationary: {cert['stationary']})"
        )
        print(
            f"    active constr  {cert['n_active_constraints']}  ->  critical cone "
            f"dim {cert['critical_cone_dim']} of 6"
        )
        print(
            f"    proj. Hessian  eig in [{cert['min_eigenvalue']:.4e}, "
            f"{cert['max_eigenvalue']:.4e}]  (noise floor "
            f"{cert['noise_floor']:.1e})"
        )
        print(
            f"    descent probe  {cert['verified_descent']:.3e}  "
            f"(curvature-driven descent found: {cert['is_saddle']})"
        )
        verdict = bool(cert["stationary"]) and not bool(cert["is_saddle"])
        print(
            f"    CERTIFIED LOCAL MIN: {verdict}   "
    ...
    print(
        f"  H1 Gram eigenvalues in [{eigs.min():.4e}, {eigs.max():.4e}], "
    ...
    def fd_gap(c, wdt, p_order, kind):
        ...
        def red(p):
            return solve_consequents(
                p[:3], p[3:], yd, dyd, quad, lam, p_order, kind, X_REF
            )[1]
    ...
    print(
        f"{'MF kind':>9} {'order':>6} {'coverage':>11} {'cond(G)':>11} "
    ...
    for kind, (c, wdt) in configs.items():
        for p_order in (0, 1, 2):
            ...
            print(
                f"{kind:>9} {p_order:>6} {cov:11.3e} {cond:11.2e} {err:13.3e} {h_at:8.0e}"
            )
    ...
    for width in (5.0, 7.0, 13.0):
        err, h_at, cond, cov = fd_gap(
            np.array([-10.0, 0.0, 10.0]), np.full(3, width), 1, "bump"
        )
        print(f"{'bump':>9} {1:>6} {cov:11.3e} {cond:11.2e} {err:13.3e} {h_at:8.0e}")
    print(
        "  => exact to ~1e-8 exactly where coverage (C2) holds, and meaningless where"
    )
    print(
        "     it fails -- because there the model is not differentiable at all.  C2 is"
    )
    ...
    print(
        "  rules the slope term is NEGATIVE (one rises where the other falls), so the"
    )
    print(
        "  two terms can cancel at a positive lambda* -- giving exact H1 orthogonality"
    )
    print("  with overlapping, C-infinity, positively-covering membership functions.")
    print()
    fine = Quadrature.legendre(X_LO, X_HI, 2000)
    print(
        "  N=2, order 0.  lambda* = -INT(phi_0 phi_1) / INT(phi_0' phi_1'), closed form:"
    )
    print(
        f"{'width':>8} {'INT phi.phi':>13} {'INT dphi.dphi':>15} {'lambda*':>11} "
    ...
    for p_order in (0, 1):
        for lam_v in (1.0, lam_star, 10.0):
            _, j_act, _ = solve_consequents(
                cen, wid, yd_f, dyd_f, fine, lam_v, p_order, "gaussian", X_REF
            )
            _, g_act = sequential_fit(
                cen, wid, yd_f, dyd_f, fine, lam_v, p_order, "gaussian", X_REF
            )
            ps, dps, _ = rule_regressors(
                fine.nodes, cen, wid, p_order, "gaussian", X_REF
            )
            obe = decoupling_report(
                h1_gram(ps, dps, fine, lam_v), 2, p_order
            ).off_block_energy
            tag = "  <-- lambda*" if abs(lam_v - lam_star) < 1e-9 else ""
            print(
                f"{p_order:>6} {lam_v:10.4f} {obe:12.3e} "
    ...
    print()
    print(
        "  Scope for N > 2 (order 0): N(N-1)/2 pairwise conditions, still one lambda,"
    )
    print("  so exact cancellation is generically impossible -- but it still helps:")
    print(
        f"{'N':>4} {'width':>7} {'off-block at lam=1':>20} {'best lam':>10} "
    ...
    for n_r in (2, 3, 4, 5):
        ...
        def off(lam_v, c=cen, w_=wid, n_=n_r):
            ...
            return decoupling_report(
                h1_gram(ps, dps, fine, lam_v), n_, 0
            ).off_block_energy

        best = minimize_scalar(
            off, bounds=(1e-3, 1e5), method="bounded", options={"xatol": 1e-8}
        )
        print(
            f"{n_r:>4} {wid[0]:7.2f} {off(1.0):20.3e} {best.x:10.3f} {best.fun:17.3e}"
        )
    ...
    print(
        "  logistic: phi_1 = sigmoid(k x) with k = 4c/b^2, because the log-ratio of two"
    )
    print(
        "  equal-width Gaussians is linear in x.  Both integrals are then elementary,"
    )
    print("    INT phi_0 phi_1 dx = 1/k        INT phi_0' phi_1' dx = -k/6")
    print(
        "  (the second by substituting s = sigmoid(u), giving INT_0^1 s(1-s) ds = 1/6),"
    )
    print("  so lambda* = 6/k^2 = 3 b^4 / (8 c^2).")
    print()
    print(
        f"{'c':>5} {'b':>5} {'k=4c/b^2':>10} {'INT pp':>10} {'1/k':>10} "
        f"{'INT dd':>11} {'-k/6':>10} {'lam* num':>10} {'6/k^2':>10}"
    )
    for c, b in ((5.0, 2.0), (5.0, 4.0), (3.0, 2.0), (8.0, 3.0), (6.0, 2.5)):
        ph, dph, _ = normalized_weights(
            wide, np.array([-c, c]), np.full(2, b), "gaussian"
        )
        pp = np.trapezoid(ph[0] * ph[1], wide)
        dd = np.trapezoid(dph[0] * dph[1], wide)
        k = 4 * c / b**2
        print(
            f"{c:5.1f} {b:5.1f} {k:10.5f} {pp:10.6f} {1 / k:10.6f} {dd:11.6f} "
    ...
    for c, b in ((5.0, 2.0), (5.0, 4.0), (3.0, 2.0), (8.0, 3.0)):
        w_c = b**2 / (4 * c)
        print(
            f"    c={c:4.1f} b={b:4.1f}  w={w_c:8.5f}  ell*={np.sqrt(lam_star(c, b)):8.5f}"
            f"  ratio={np.sqrt(lam_star(c, b)) / w_c:8.5f}  (sqrt 6 = {np.sqrt(6):.5f})"
        )
    print()
    print(
        "  So lambda* is fixed entirely by the PARTITION GEOMETRY.  y_d appears nowhere"
    )
    print(
        "  in it.  It is therefore not a distinguished correlation length of the target"
    )
    print("  -- it is the correlation length of the rule crossover region.")
    print()
    print(
        "  WHY N>=3 CANNOT WORK.  lambda* ~ 1/separation^2, so on a uniform partition"
    )
    ...
    for d, b in ((6.0, 3.0), (8.0, 4.0), (10.0, 4.0)):
        ...
        print(
            f"    pitch={d:5.1f} b={b:4.1f}  lam*_adjacent={la:9.5f}  "
    ...
    print(
        f"{'c':>5} {'b':>5} {'w':>8} {'lambda*':>10} {'ell*':>8} {'L2 @ 0':>9} "
        f"{'L2 @ lam*':>10} {'cost':>8}"
    )
    for c, b in (
        (5.0, 1.0),
        ...
        ls = lam_star(c, b)
        ps, dps, _ = rule_regressors(
            q_t.nodes, np.array([-c, c]), np.full(2, b), 0, "gaussian", X_REF
        )
        errs = []
        for lam_v in (0.0, ls):
            gm = h1_gram(ps, dps, q_t, lam_v)
            th = np.linalg.solve(
                gm + 1e-12 * np.trace(gm) / 2 * np.eye(2),
                h1_project(ps, dps, yt, dyt, q_t, lam_v),
            )
            errs.append(np.sqrt(q_t.integrate((yt - ps.T @ th) ** 2)))
        print(
            f"{c:5.1f} {b:5.1f} {b**2 / (4 * c):8.4f} {ls:10.4f} {np.sqrt(ls):8.4f} "
