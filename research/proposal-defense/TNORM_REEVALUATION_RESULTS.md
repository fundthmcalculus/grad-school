> **Recovered 2026-08-30 from the pre-rewrite history — read this first.**
> The 2026-08-20 history rewrite dropped this file, but it is still cited by
> `reproduce/manifest.py:360` and `reproduce/PROVENANCE_MAP.md`'s `table_norm_conorm_matrix` row, so the citation was dangling. It is restored **verbatim**: nothing
> in it has been re-verified, no number in it has been re-measured, and where
> the file carries its own supersession notes those still apply and take
> precedence over anything above them. Treat it as a record of what was
> thought at the time, not as current guidance. The unmodified original is on
> `archive/pre-rewrite-docs-ch7-split-to-appendix`.
