...
- ⬜ **Ch 3 — TUNED DENSE-PRIM BASELINE (the strongest baseline, currently missing).** All Ch3 speedups are measured against the *cubic* textbook VAT. A properly implemented dense Prim (maintaining a best-distance array) is O(N²) and asymptotically BEATS pVAT's O(N² log N), so the ratios cannot be quoted as pVAT's advantage over the state of the practice until this arm exists. §3.3.1 now states this openly and rests the chapter's claim on feasible problem size (memory) rather than the speedup ratio. Add the arm to `reproduce/tables/table_3_1_pvat_scaling.py`.
