...
- 🟨 **Ch 4 — OUTPUT PARTITIONING STUDY: RAN on Concrete, partly answered (Goal G5).** `reproduce/tables/table_g5_output_partitioning.py`, 3 schemes × {3,4,6} buckets × {1st,2nd} order × 3 seeds.
  **H4 SUPPORTED — crossover near 4 buckets.** Uniform wins at 3 (R² 0.811 vs 0.797), ties at 4, loses at 6 (0.842 vs 0.850). Mechanism is visible in the starvation diagnostic: uniform's smallest bucket falls 132 → 75 → 39 as the partition refines while quantile's floor stays high by construction. Uniform loses to *data exhaustion at the sparse end*, not to bad geometry.
  **H3 weakly supported** — uniform holds the tail deciles slightly better, small and not consistent in every cell.
  **H5 FALSIFIED, and it is a code finding.** The "hybrid" (quantile boundaries + extreme centroids pinned to min/max) is what `partition_output` ALREADY does — and it is **bit-identical to pure quantile in all 18 configurations**. Reason: `solve_tsk_consequents` returns its own re-optimized `y_bucket_mean_opt`, and `predict_tsk` uses that, so the pinned values are discarded before they matter. **The pinning is dead code on the TSK path.** Either remove it or give the solve an option to hold the extreme centroids fixed, which is presumably what it was written to do.
  **H2 STILL UNTESTED** — Concrete's skew is only +0.42. The central hypothesis (quantile's advantage grows with skew) needs a genuinely skewed real target plus the synthetic skew sweep. That is what remains of G5.
...
- 🟨 **Ch 4 — anomaly/open-set head-to-head: RAN, mechanism validated, performance not.** `reproduce/tables/table_4_4_openset.py`, leave-one-class-out on Glass (BETH absent from the repo), 2 seeds.
  **Validated:** the θ knob is monotone exactly as §4.3.5 predicts — raising θ shrinks the complement and cuts both detection and false alarms, saturating at θ ≥ 1.1 where the rule stops firing entirely. Best separation is at **θ = 0.80 (J = +0.155)**, roughly double the **θ = 0.99** default inherited from `beth-anomaly.py` (J = +0.075). Argues for reporting the curve and tuning θ per deployment.
  **Not validated:** absolute performance is poor (best ≈ 31% detection at 13% false alarm) and the complement rule's nominal lead over isolation forest and one-class SVM is smaller than the across-class spread, so the three are indistinguishable on this evidence. Glass (214 samples, 6 classes, several with <12 members) is a stress test, not a demonstration.
  **Next:** obtain BETH or an equivalent-scale set; re-run with ≥5 seeds; then the claim can move from parity to a comparison.
- ⬜ **Ch 4 — original head-to-head note (Table 4.3):** complement rule vs **one-class SVM** and **isolation forest** on BETH (train on benign only, detect unseen `evil==1`). Report detection rate + false-alarm rate at a matched operating point, plus the θ sweep curve (Fig 4.2, from `plot_anomaly_threshold_sweep`). This is the experiment owed to the §4.3.5 claim.
