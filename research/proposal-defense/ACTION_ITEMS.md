...
- 🟨 **Tables (21 total; 28 `*pending*` cells remain).** Fully measured: 3.2 (memory), 3.3 (adversarial ARI), 3.4 (stitch ablation), 4.1 (MoG results), 5.1 (multi-scale), 5.2 (selection bake-off), 6.1 (model family), 7.1 (goals map). Structure-fixed with pending cells: 3.1 (reorder time — intermediate N grid), 4.2 (ANFIS/GA-FIS/RF baselines), 6.2 (CART/M5/RF/ANFIS/flat-TSK baselines), 6.3 (interpretability counts at matched accuracy).
- 🟨 **Reproduction harness** (`reproduce/`): `run.py` orchestrator + 19 registered experiments, **6 verified running end-to-end** (three-arm reorder, Concrete reconciliation, hyperparameter×normalization, open-set, G5, G5b). Emits Markdown+CSV with mean ± std over fixed seeds; hardware-gated runs skipped explicitly. REMAINING: ANFIS/GA-FIS adapters; a full `--all` pass; fill the remaining `*pending*` cells.
...
- ✅ **Ch 4 — OUTPUT PARTITIONING STUDY: COMPLETE (Goal G5).** Quantile by default; Ch4 Tables 4.2–4.3. `reproduce/tables/table_g5_output_partitioning.py`, 3 schemes × {3,4,6} buckets × {1st,2nd} order × 3 seeds.
  ...
  **Caution:** past skew ~16 both schemes fail (quantile R² goes negative with σ≈1), so the apparent uniform win in the last two rows is noise between two broken models, not a result — at that point the target needs a transform, not a better partition.
  **G5 is complete.** Recommendation: quantile by default. Only corroboration on a skewed *real* target remains, as a nice-to-have.
