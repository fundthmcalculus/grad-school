...
- 🟨 **Ch 4 — anomaly/open-set head-to-head: RAN, mechanism validated, performance not.** `reproduce/tables/table_4_4_openset.py`, leave-one-class-out on Glass (BETH absent from the repo), 2 seeds.
  **Validated:** the θ knob is monotone exactly as §4.3.5 predicts — raising θ shrinks the complement and cuts both detection and false alarms, saturating at θ ≥ 1.1 where the rule stops firing entirely. Best separation is at **θ = 0.70 (J = +0.261)**, about half again the **θ = 0.99** default inherited from `beth-anomaly.py` (J = +0.170). Argues for reporting the curve and tuning θ per deployment.
  **Not validated:** absolute performance is a real but noisy signal rather than a deployable detector (best ≈ 61% detection at 35% false alarm), and at the matched θ = 0.99 point isolation forest nominally leads (J = +0.208) the complement rule (+0.170) and one-class SVM (+0.062) — a reversal of the pre-fix ordering, and still smaller than the across-class spread, so the three remain indistinguishable on this evidence. Glass (214 samples, 6 classes, several with <12 members) is a stress test, not a demonstration.
  **Superseded:** the figures above are from `reproduce/outputs/openset-postfix/` (tribble-fis `23bfdbc`, 5 seeds). The earlier "best J = +0.155 at θ = 0.80" reading was a 2-seed, pre-`pin_extremes` run and is retired.
