...
- ⚠️ **BROKEN CITATION — resolve `kdtreevat2024`.** Ch2 §2.2 and Ch3 §3.2 cite the kd-tree/BB-VAT memory methods as "[*Information Sciences* 2024]" — journal name and year only, no author/title/DOI. Will not survive review, and it is load-bearing: that line achieves *sub-quadratic* memory for Euclidean data, which is what makes Ch3's "unoccupied regime" argument precise. Flagged `[!]` in `references.bib`.
...
- ⬜ **NEW PAPER — VAT/iVAT sequencing complexity, with Dr. Kreinovich** (Ch 9, scheduled 2027 Q1). Implementation exists (`pcvat.pyx::_prim_mst_kernel_64/_32`); the work is the write-up plus a THREE-ARM timing study (classical cubic vs stage-one heap vs stage-two dense).
