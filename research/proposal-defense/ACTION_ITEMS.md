...
- ⬜ **BREAKING (upstream) + a correction the "log+std" finding owes.** `tribble-fis` [PR #67](https://github.com/fundthmcalculus/tribble-fis/pull/67) replaces the ad hoc `gauss_math.detect_and_apply_log_transform`/`standard_transform` helpers (which every "log+std" table above actually calls) with two proper sklearn transformers in `tribblefis.scaling`: `StandardScalar` (z-score, `mu=0`/`sigma=1`) and `UnitScalar` (min-max to `[0,1]`, the direct successor of `standard_transform`). **Two things follow:**
  1. **Migration required, not optional.** `standard_transform`/`detect_and_apply_log_transform` are deleted outright (dead/broken code upstream, see the PR). Seven files here import them directly and will break on an ImportError the moment the `tribble-fis` pin is bumped past this PR: `FuzzySystemsExperiments/concrete.py`, `FuzzySystemsExperiments/nasa.py`, `reproduce/tables/table_concrete_reconciliation.py`, `reproduce/tables/table_hyperparam_normalization.py`, `reproduce/tables/table_g5_output_partitioning.py`, `reproduce/tables/table_g5b_skew_sweep.py`, `reproduce/tables/_fuzzy_models.py` (comment only, but check for latent calls). Swap in `UnitScalar(log_dynamic_range=...)` for an equivalent transform (note the old calls pass `min_dynamic_range=2`; `UnitScalar`'s default is `3.0`, so pass `log_dynamic_range=2` explicitly to reproduce the existing numbers exactly).
  2. **"log+std" was never z-score.** `standard_transform` is a min-max scaler ((X−min)/(max−min)) despite the name — every "log+std"/"standardized" result quoted above (the +0.06 to +0.15 R² finding, the Concrete reconciliation, the hyperparameter×normalization table) is really **log + `[0,1]` bounding**, i.e. what is now `UnitScalar`. **True z-score standardization (`StandardScalar`) has never actually been measured against it.** Once the migration above lands, re-run `table_hyperparam_normalization.py` (and ideally `table_concrete_reconciliation.py`) with a third arm: raw / log+`UnitScalar` (as now) / log+`StandardScalar`. This is the natural place to settle which normalization the dissertation should recommend as the FIS default — currently an open, undecided choice upstream.
