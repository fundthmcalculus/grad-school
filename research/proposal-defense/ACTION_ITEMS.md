...
- ⬜ **NEW PAPER — $O(N^2)$ VAT/iVAT sequencing, with Dr. Kreinovich** (Ch 9, scheduled 2027 Q1). Implementation exists (`pcvat.pyx::_prim_mst_kernel_64/_32`); what's needed is the write-up plus a three-way timing study (classical cubic vs stage one vs stage two). Keep the novelty scoped: dense Prim is textbook — the claim is the VAT-sequencing formulation, the fused single pass, O(N) workspace, and bit-identical verification against the serial reference.
