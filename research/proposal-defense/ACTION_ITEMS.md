...
- ⬜ **Ch 3 — NOMENCLATURE DECISION (author review pending).** "pVAT = priority-queue VAT" names the SLOWER fallback, not the O(N²) kernel that produces the headline numbers. Options: (a) re-read the p as parallel/performant (also covers Borůvka-on-GPU), (b) name the two paths distinctly throughout. Author wants to review the kernel personally before deciding.
