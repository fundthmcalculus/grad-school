...
1. **Produce the remaining 15 figures.** One of sixteen exists — Figure 3.2, the complexity fit, generated by the harness in PNG and EPS. Two are load-bearing: **Fig 1.2** (pipeline roadmap — orients the whole document) and **Fig 5.2** (band discovery on the log-birth spectrum — carries Ch 5's contribution). The rest are supporting. This is the single largest visible gap in the draft.
