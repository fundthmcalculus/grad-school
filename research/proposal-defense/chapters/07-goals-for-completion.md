...
- **Goal:** a single end-to-end system — Data → pVAT structure (Ch 3) → persistence/multi-scale selection + MF generation (Ch 5) → FIS synthesis (Ch 4/Ch 6) → optional structure-driven refinement (Ch 2/Ch 6) → interpretable FIS — demonstrated on real, large, non-metric data.
...
- Applies to: Ch 3 pVAT scaling/memory/GPU numbers; Ch 5 selection-pipeline scaling; Ch 6 training-time/accuracy/speedup numbers.
- Plus: head-to-head vs eVAT & clusiVAT on identical datasets; distributed pVAT toward 500K elements.
