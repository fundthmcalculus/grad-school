...
**Name:** **mergeVAT (priority-queue VAT)** — naming centered on the priority-queue argmin (CPU) and Borůvka MST (parallel/GPU). Head-nod to Dr. V. Kreinovich, who observed the method is really a priority-queue/heap approach. Originally "mergeVAT" (see naming callout in §3.3.1); NAFIPS paper 2 published under the "mergeVAT" title — keep that title in Ch 9.
...
### 3.3.1 The O(N²log N) reorder (mergeVAT) + naming callout
...
- **Before-submission checklist:** thermally-stable re-timing + error bars; datacenter GPU full-rate FP64; head-to-head vs eVAT & clusiVAT on identical data; drop the ungrounded "mergeVAT six-orders-of-magnitude" web claim (self-citation at most); verify DOIs.
