...
|---|---:|---:|---:|
| 1st | 0.797 | **0.830** | +0.033 |
| 2nd | 0.845 | **0.872** | +0.027 |
| full 2nd | **0.881** | 0.854 | **−0.027** |

Refinement buys about three points of $R^2$ at first and second order, which is real but modest. At full second order it *costs* about the same amount, and the best model in the whole table is the unrefined closed-form full-2nd solve at 0.881. An earlier draft of this chapter reported refinement lifting Concrete from roughly 0.88 to 0.92; that figure is not reproduced, and 0.92 does not appear anywhere in a controlled run. I am striking it.

The shape of the correction is worth more than the number. Refinement helps when the consequent model is relatively low-capacity and hurts once the model is expressive enough to fit the data well on its own — which is the same pattern §6.3.5 already reported for the population methods, where differential evolution and a genetic algorithm overfit the cross-validation estimate and a plain local optimizer beat them. Both observations say the same thing: once the structure has been recovered from the data, additional search finds noise rather than signal, and at high capacity it finds enough noise to do damage. That is an argument for *structure before search* made against my own refinement stage, which is the version of the argument I trust most.
