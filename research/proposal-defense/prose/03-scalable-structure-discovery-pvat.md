...
That $O(N^2 \log N)$ is not the end of the story, and it is worth laying out the full picture because the implementation is better than the priority-queue framing suggests. There are three complexity regimes here, not two.

The **classical textbook reorder** is $O(N^3)$, for the re-scan reason above. This is what the reference implementations in this literature do, and it is what makes VAT unusable past a few thousand points.

The **priority-queue reorder** is $O(N^2 \log N)$. Each accepted vertex triggers an $O(N)$ relaxation pass over the remaining candidates, and each improved candidate is pushed onto a heap; with lazy deletion the heap can hold $O(N^2)$ entries, so the $\log$ factor comes from the heap operations rather than from the scanning. This is the path the pure-Python and Numba implementations take, and it is the one the name pVAT was coined for.

The **compact-active-set dense reorder** is $O(N^2)$, with no heap at all, and this is the production path. It keeps the unvisited vertices packed into the first $m$ slots of a small set of parallel arrays; removing a vertex is an $O(1)$ swap with the last active slot, so round $r$ scans only $m = N - r$ entries. Crucially it *fuses* the relaxation and the next-minimum selection into a single pass over those $m$ entries — update each candidate's key against the newly added row, and track the running minimum in the same loop — so each round costs one pass, not two. Summing $N - r$ over all rounds gives $N^2/2$ comparisons and $O(N)$ workspace beyond the matrix itself. The per-row work the classical version wastes is exactly what this eliminates.

So the sharp version of the earlier caveat is not that I owe a dense-Prim baseline. It is that **the dense $O(N^2)$ Prim is already the shipped fast path**, and the heap version is the portable fallback. Both produce bit-identical orderings. The honest accounting is therefore: against the cubic implementations that exist, the improvement is a factor of $N/\log_2 N$ for the fallback path and $N$ for the compiled path; against a hypothetical well-tuned competitor, the compiled path is already at the same $O(N^2)$ complexity class, and what distinguishes it is the $O(N)$ workspace and the fused inner loop rather than the exponent.

> **Nomenclature, flagged for revision.** This creates a naming problem I should not paper over. "pVAT," read as *priority-queue VAT*, describes the $O(N^2 \log N)$ fallback — not the $O(N^2)$ kernel that actually produces the headline numbers. Either the name should be read more broadly (parallel, or performant, VAT — which also covers the Borůvka path on the GPU) or the chapter should distinguish the two paths by name throughout. I am leaving this open pending review of the kernel itself; the code is cited in §3.4 so the claim can be checked directly rather than taken on my word.
...
| compiled path, $O(N^2)$ dense | — | *(is the fast path)* | ~$N$ vs cubic | **shipped kernel**, not a baseline |

The intermediate grid of $N$ is swept by the harness rather than tabulated here. Every speedup above is measured against the cubic implementation; as §3.3.1 explains, the compiled path is itself already an $O(N^2)$ dense Prim, so these ratios describe the gain over the common implementation rather than over the best achievable complexity class.

**Implementation, for verification.** The complexity claims above are checkable against the source rather than taken on trust. The $O(N^2)$ compact-active-set kernel is `_prim_mst_kernel_64` in [`src/tribbleclustering/pcvat.pyx`](https://github.com/fundthmcalculus/clustering/blob/c71171e/src/tribbleclustering/pcvat.pyx#L22-L113) (lines 22–113; the `float32` twin is `_prim_mst_kernel_32` at line 392), whose fused relax-and-select inner loop is the `for i in range(m)` block. The $O(N^2 \log N)$ heap fallback is `vat_prim_mst` in [`src/tribbleclustering/pvat.py`](https://github.com/fundthmcalculus/clustering/blob/c71171e/src/tribbleclustering/pvat.py#L141-L211) (lines 141–211). The compiled path is preferred at import time and the heap path is the fallback, so the shipped behaviour is the $O(N^2)$ one wherever the extension is built.
