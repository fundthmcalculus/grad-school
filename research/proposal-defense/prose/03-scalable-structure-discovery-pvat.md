...
What remains is a claim about the literature, not about the algorithm, and a prior-art search has narrowed it considerably; my first version of this argument was too broad.
...
**A note on the name.** The method is called **mergeVAT**, and the name has been round-trip. I chose it first for an idea I was chasing then: structuring the reordering as something like a two-dimensional merge sort. Dr. Vladik Kreinovich then pointed out that what the code actually did was far closer to a heap sort: it was a priority-queue algorithm. He was right about stage one, and I renamed it *pVAT* on the strength of it. That name turned out to be taken. Parveen and Sreevalsan-Nair published a **pVAT** in 2013: *parallel* VAT, on a GPU, likewise swapping the MST algorithm used for the ordering, Prim for Borůvka [Parveen and Sreevalsan-Nair 2013, BDA, LNCS 8302:151–170]. That is close enough in both name and subject to cause real confusion, and reading my *p* as *parallel* or *performant* would collide harder, not less, since their method is parallel and mine also has a GPU path.

So the name returns to mergeVAT, and it is not ideal. It describes neither of the two things §3.3.1 is about: stage one is a priority queue, stage two a compact active set, and neither is a merge. A reader meeting the name cold will not derive the reorder from it.

What the name does describe is §3.3.4. Splitting a problem into blocks, ordering each, and stitching the results back into one faithful ordering is a merge in the ordinary sense, and it is the part of this work that reaches past a single machine. It is also the least finished. The principled stitch works and is measured: Table 3.6 shows it recovering an adjusted Rand index of 1.00 where the naive concatenation collapses. But it is a two-way stitch over blocks chosen by farthest-point sampling, not a general merge operator, and I do not yet know how it behaves under the repeated or hierarchical application a distributed implementation would need. Chapter 7 carries that as open work, not as a finished component.

So the name points at the piece of the method with the most research left in it, not at the piece that is done. I would rather keep a name that is imperfect and stable than rename a method twice more while the work underneath it settles.
...
Every one of those ratios is a this-host measurement, and the qualifier is load-bearing rather than decorative. The development-laptop archives put the pure-Python ratio at 1,116–1,146× where four independent runs on this host put it at 660–704×, and the compiled comparison at 11–50× there against 10–207× here, the shape changing rather than the scale: on the laptop the ratio *fell* with $N$ over the swept grid and here it rises monotonically. Quoting a laptop ratio beside a table of workstation numbers would contradict the claim two paragraphs above that every CPU number in this section comes from one host. All of them are also measured against the cubic implementations that exist in this literature, not against a tuned $O(N^2)$ dense Prim, which would narrow the gap considerably.
...
**Why this table reports ratios and not seconds, and how far that goes.** The normalization is a deliberate reporting standard, not a presentational choice. On the development laptop, a four-core i7-1185G7 with 16 GB under a `powersave` governor, the same generator at the same `tribble-cluster` commit and the same ten seeds ran three times in one afternoon: the 1,024-point classical arm came back at 22.2 s, then 31.7 s, then 21.3 s, a spread of nearly 50%, while the ratio between the arms moved only across 1,146×, 1,116× and 1,129×, under 3%. That is the case for quoting ratios, and it stands.

What that does *not* license is the stronger reading, that a ratio between two arms is a property of the algorithms and so survives a change of machine. **It does not, at least not this pair of arms.** On the workstation three runs put the 1,024-point classical arm at 13.7, 14.2 and 14.5 s, a spread of 6%, and the ratio at 660×, 673× and 700×. That is 40% away from the laptop's 1,129×. The classical reference is interpreted Python and mergeVAT is compiled Cython, so a change of host does not scale the two arms by the same factor, and their quotient moves with it. A ratio is invariant to a shared constant factor, and these two arms do not share one.

So the standard this chapter reports under is the weaker and still useful version: **within a host the ratio is far more stable than the seconds (6% against 40% here), and across hosts it is more portable than seconds without being invariant.** The chapters quote ratios, the harness keeps the seconds and their per-seed spreads in the companion CSV, and a ratio quoted from one host is read as that host's measurement and not as a property of the algorithms.

The exponents of Table 3.2 are the most portable quantity here, and that is the argument for leaning on them rather than on any speedup: an exponent is invariant to a constant factor, which no ratio between these two arms is. They are not machine-independent either, and this chapter contains its own counterexample: the same code fitted 2.12 for stage two on the laptop and 1.97 here. What travels is the qualitative separation, cubic against quadratic, which both hosts agree on. Portability is a matter of degree, and every level of the ladder from seconds to ratios to exponents earns its place separately.

The laptop's ~50% run-to-run swing was thermal and specific to that machine: it does not reproduce here, where repeats agree to 6%.
...
One methodological point carries out of this section. **Determinism across runs says nothing about invariance across machines**: the ratios above are repeatable to 6% on either host and still 40% apart between them. That is why Goal G4 asks for a fixed protocol and not merely for error bars, and why this chapter's numbers come from one identified host with its numeric stack recorded.

The 4,096-point row is the NAFIPS measurement, carried over, since the reproduction harness does not produce it. The harness caps its own cubic reference at $N \leq 1{,}024$ (`REPRO_NAIVE_CAP`); lifting the cap to 4,096 is a single flag, but the cubic arm costs roughly sixty-four times as much at four times the size, several hours of compute to re-derive a constant factor. That is a poor trade, because the constant factor is not the claim. What this section argues is the *scaling*: that the exponent drops from cubic to quadratic, and that the feasible problem size moves from the few-thousand range these implementations are used at to a measured 135,000 points, as stated precisely above. The 4,096 row illustrates a ratio at one convenient size; it is not the evidence for that argument. The evidence is Table 3.2, which pins **both** exponents on the host of record, cubic at 3.20 and quadratic at 1.97, each stable across five runs. Running the whole grid on one host is what lets this section defend the exponent drop as well as the reachable problem size.
...
The in-place scheme buys a factor of $\sqrt{3}$ in reachable $N$ over the classical three-matrix scheme, at every precision and under either budget: that is the memory contribution to §3.4's scale claim. Halving the precision buys another $\sqrt{2}$, and at $N = 2{,}000$ across ten seeds the float32 ordering is elementwise identical to the float64 one. That is a check at one size, and the smallest interesting one. Dropping precision collapses near-equal dissimilarities into equal ones, and ties are what make a minimum spanning tree non-unique, so $N = 2{,}000$ is the size least likely to expose the failure. I state the single-precision row as arithmetic plus a small-$N$ ordering check, not as a free doubling, until the check is repeated at scale (Goal G4). The two budget columns bracket the 135,000-point run: impossible under the cap, unremarkable on the full machine. That is the whole content of that row in Table 3.1. And the on-demand row is a negative result, its ceiling arithmetic about a scheme that does not currently work, which is precisely why it is printed rather than omitted. Precision below float32 is deliberately out of scope here: the CPU kernels ship at 64 and 32 bits, and half precision belongs with the Borůvka/GPU path of §3.3.3, where it would actually pay.
...
The pairwise-distance row is the one worth dwelling on. On this hardware the GPU is slower than the CPU for low-dimensional or double-precision distance computation, for the reason §3.3.3 gives. That is a property of the card and not of the algorithm, and it is why the repeatability protocol calls for a datacenter GPU: on a card with full-rate FP64 this row would likely flip. I flag that as an untested prediction rather than quietly present laptop numbers as the method's ceiling.
...
| Real non-coordinate domains (DTW, edit distance, graph kernel) | no | — | *not run — no non-coordinate dataset in the harness (Goal G2)* |
...
There is work left before this is airtight as a journal result, and I name it rather than let a committee find it. The CPU timings come from one identified host at ten seeds with error bars, reproduced across independent runs, and §3.4 measures how far a ratio moves when the host changes, which is why the remaining hardware item matters. Clocks and thermals are still not pinned. The GPU story needs a datacenter card with full double-precision throughput to separate the algorithm from the consumer card's penalty. And the non-metric claim, the heart of the niche, is so far demonstrated on synthetic non-metric dissimilarities; it needs a real non-coordinate domain, time series under dynamic time warping or strings under edit distance, to be fully convincing. That last item is a goal for completion in Chapter 7. Finally, I owe the reader a direct head-to-head against eVAT and clusiVAT on identical datasets, which I have not yet run and which is the first comparison a reviewer will want.
