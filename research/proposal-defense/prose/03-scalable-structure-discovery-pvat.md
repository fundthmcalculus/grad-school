...
**A note on the name.** The method is called **mergeVAT**, and the name has been round-trip. I chose it first for an idea I was chasing at the time — structuring the reordering as something like a two-dimensional merge sort. Dr. Vladik Kreinovich then pointed out that what the code actually did was far closer to a heap sort: it was a priority-queue algorithm. He was right about stage one, and I renamed it *pVAT* on the strength of it. That name turned out to be taken. Parveen and Sreevalsan-Nair published a **pVAT** in 2013 — *parallel* VAT, on a GPU, which likewise swaps the MST algorithm used for the ordering, Prim for Borůvka [Parveen and Sreevalsan-Nair 2013, BDA, LNCS 8302:151–170]. That is close enough in both name and subject to cause real confusion, and reading my *p* as *parallel* or *performant* would collide harder rather than less, since their method is parallel and mine also has a GPU path.

So the name returns to mergeVAT, and I will say plainly that it is not an ideal one. It describes neither of the two things §3.3.1 is actually about: stage one is a priority queue and stage two is a compact active set, and neither is a merge. A reader meeting the name cold will not derive the reorder from it.

What the name does describe is §3.3.4. Splitting a problem into blocks, ordering each, and stitching the results back into one faithful ordering is a merge in the ordinary sense, and it is the part of this work that reaches past a single machine. It is also the least finished part. The principled stitch works and is measured — Table 3.6 shows it recovering an adjusted Rand index of 1.00 where the naive concatenation collapses — but it is a two-way stitch over blocks chosen by farthest-point sampling, not a general merge operator, and I do not yet know how it behaves under repeated or hierarchical application, which is what a distributed implementation would need. Chapter 7 carries that as open work rather than as a finished component.

That is the honest reading of the name: it points at the piece of the method with the most research left in it rather than at the piece that is done. I would rather keep a name that is imperfect and stable than rename a method twice more while the work underneath it settles.
...
**Table 3.1 — Reorder time, classical VAT vs. mergeVAT.** Normalized against the slower arm in each row: the classical reference is the 1.0× baseline and mergeVAT reads as how many times faster it is. Absolute seconds, with per-seed spreads, are in the harness CSV. The largest sizes are mergeVAT-only because the classical reference is infeasible there, so those rows have nothing to normalize against.
