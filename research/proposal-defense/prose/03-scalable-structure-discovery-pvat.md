...
I want to be precise about what is and is not novel in stage two, because the ingredients are old. Maintaining an array of best-distances-to-tree is textbook dense Prim, and dense Prim has been $O(N^2)$ since 1957. What I claim is narrower: the formulation for *VAT sequencing specifically* — the fused single-pass relax-and-select, the compact active set that shrinks the scanned region each round, and the demonstration that this yields orderings bit-identical to the serial reference in $O(N)$ workspace, which is what makes it compose with the on-demand distance computation of §3.3.2. That combination is the subject of the paper described below, and I would rather scope the claim tightly than let a reviewer scope it for me.
