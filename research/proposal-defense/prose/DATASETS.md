...
- **Status:** **DEMONSTRATED (single-run), not measured.** DS02 per-sample RMSE 6.48 in ~1 s beats the public-file CNN (7.22) / MLP (8.34) re-runs on their 20-channel input set; real-sensors-only matches the virtual-channel best when pooled. **Not redistributable** — the 10 `.h5` files total ~28 GB and are gitignored; only the scripts and the committed report `cmapss_rul_full_analysis_report.md` are tracked. Promotion to the ten-seed harness protocol is `CHECKLIST` **C14**; primary-source verification of the baseline figures is **C15**.
