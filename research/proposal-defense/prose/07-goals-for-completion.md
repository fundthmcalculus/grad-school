...
**G5 — Output partitioning, mostly settled.** The quantile-versus-uniform question of Chapter 4 §4.3.2 has been run on Concrete and answered in part: there is a crossover near four buckets, driven by bucket starvation rather than by geometry, and the "hybrid" third option turns out to be what the code already does with its distinguishing feature inert. What remains is the skew axis. Concrete's target skew is only +0.42, so the central hypothesis — that quantile's advantage grows with skew — is still untested. Completing G5 means a genuinely skewed real target plus a synthetic sweep that varies skewness directly to locate the crossover as a function of it.
