...
**G5 — Output partitioning: settled (complete).** The quantile-versus-uniform question of Chapter 4 §4.3.2 is answered and no longer proposed work. Quantile boundaries are the default: indistinguishable from uniform on a symmetric target, worth up to 0.2 in $R^2$ on a skewed one, and structurally immune to the failure that sinks uniform. That failure is bucket starvation — a synthetic sweep isolating skew as a single parameter shows uniform's smallest bucket falling to one sample by skew 1.9 and to zero past skew 16 — and it is invisible to aggregate error until the model has already broken. The study also corrected one of my own predictions (uniform does *not* hold the tails better; an evenly spaced bucket in a sparse tail is useless if nothing lands in it) and surfaced a bug in the consequent solve, now fixed upstream. What remains is only corroboration on a skewed *real* target, which is a nice-to-have rather than a gap.
