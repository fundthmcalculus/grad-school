...
It is not built: the package's only matrix-free implementation does not produce the VAT ordering at all, and Table 3.3 reports that as a measured negative result (Appendix A.6). Nothing in Chapter 3 depends on it, since the in-place scheme at single precision reaches about 155,000 points, covering every result reported. G4d does *not* include a dense-Prim baseline: per §3.2 the compiled kernel already is one.
...
3. **Downstream usefulness against the baselines that can run — NOT MET on the letter of the threshold, though the direction favors this work.** Measured on three of the five named sets (ECG5000, Crop, FordA; ElectricDevices and StarLightCurves not attempted). Only two of the three land within 0.05 ARI of NERFCM-given-$k$, and both are low-information: on Crop (0.064 vs 0.029 ARI) both methods are weak in absolute terms on 24 true classes; on FordA (0.002 vs 0.000) every method tested scores near zero, a degenerate tie rather than agreement on a working answer. On ECG5000 — the one dataset with real recoverable structure — `select_coverage_cover` *beats* NERFCM-given-$k$ by 0.122 ARI (0.715 vs 0.593), which fails the criterion because it overshoots the tolerance on the favorable side, not because the set-cover underperforms. `select_coverage_cover` and `select_multiscale` did run on real dissimilarity matrices for the first time, closing that half of the item; the ARI-parity count itself is not yet three of five, so this item stays open, not refuted — the refutation condition (missing 0.05 on *every* real set) plainly did not happen either.
...
**Net: three of four decision-rule items are settled, one favorably-but-narrowly and one on two of three sub-cases; the fourth (downstream usefulness) is real, partial evidence in both directions rather than a pass or a refutation.** Full numbers, including the ones that do not flatter the claim (FordA's low violation rate, ECG5000's threshold-missing overperformance), are in Table 3.7, Appendix A.7.7, and `reproduce/outputs/SESSION_FINDINGS_2026-08-12.md`.

**Timeline:** Ch 10 shows 2027 Q2–Q3 (two-quarter run). **Datasets:** Appendix A.7.7 lists verified loadable datasets, three of six now measured; A.5 explains why there are no baseline competitors for exactness, though item 3 above found four that run on a dissimilarity matrix without needing coordinates.
