...
G4 also covers the head-to-head against eVAT and clusiVAT that Chapter 3 owes, and a push toward a distributed pVAT at half a million points. Two items moved into it while the chapters were being checked. The first is the **exponent itself**: Table 3.2 fits the shipped arms at 1.71 and 1.59, below the quadratic floor, because at this grid they run in milliseconds and the timer rather than the algorithm dominates. A wider grid on a pinned host is what would make §3.3.1's complexity claim measurable rather than asserted, and it is cheap. The second is the **matrix-free reorder**, which §3.3.2 had described as built. It is not: the package's only matrix-free implementation does not produce the VAT ordering at all, and Table 3.3 now reports that as a negative result. Nothing in Chapter 3 depends on it — the in-place scheme at single precision reaches about 155,000 points on the host, which covers every result reported — but writing and verifying it is what would carry the method past that, so it belongs here rather than in the completed work. It does *not* include a dense-Prim baseline: a code review established that the compiled kernel already is one, which is why Chapter 3 rests its claim on reachable problem size rather than on the exponent.
