...
| N-CMAPSS DS02 turbofan RUL | 6.5M raw rows → aggregated | **demonstrated** (single-run, `FuzzySystemsExperiments/cmapss_rul_full_analysis.py`, not the harness): DS02 per-sample RMSE 6.48 in ~1 s beats public-file CNN 7.22 / MLP 8.34; real-sensors-only matches virtual-channel best when pooled | large / scale — physics/prognostics domain, §4.4.1 / Table 4.10; promotion to ten seeds is **C14**, data not redistributable (~28 GB) |
