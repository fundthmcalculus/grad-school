...
#### The hybrid bucket arm (Chapter 4 §4.3.2)

*Is there a third option that dominates both, equal-frequency boundaries with the two extreme bucket centroids pinned to the observed min and max?* `partition_output` implements exactly that, and the sweep ran all three arms to find out. It is not a third option. It is a defect: the closed-form consequent solve re-derives its own bucket means, the prediction path uses those, and the pinned values are discarded before they can reach inference.

Reading the bucket means rather than the scores is what turned that up, because the scores cannot see it. In the run of record the largest $R^2$ separation between the hybrid and pure quantile anywhere in the sweep is 0.004, against the same arms' seed deviations of ±0.018 to ±0.027, and 0.004 is also the bound across every archive under `reproduce/outputs/`. The two arms are not identical: they differ in every one of the six bucket-and-order configurations, on at least one of the four reported metrics, and the same holds in nine other archived runs at five seeds and at ten. What they do not differ by is more than noise, which is why no accuracy number in Table 4.2 can argue the pin is working and why the defect lived as long as it did. Its consequence was a rule base saying "THEN output is $-0.81$" about a quantity min-max scaled to $[0,1]$; §4.3.2 carries the upstream fix. `ACTION_ITEMS.md` §H5 is the working record of this side quest, including the corrected bound: it originally recorded the two arms as bit-identical in all eighteen configurations, which is the claim that turns out to hold in no archive.
