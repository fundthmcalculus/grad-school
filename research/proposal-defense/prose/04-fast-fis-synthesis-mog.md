...
|---|---|---:|---:|---|
| PhiUSIIL (binary classification) | 235K × 54 | 0.65 ± 0.02 s | 0.997 ± 0.001 acc. | 2 rules (K = 2) |
...
**Table 4.5 — Baseline comparison** *(structure fixed; cells to be filled by the reproduction harness).* The speed claim is only persuasive against the methods it displaces, so this is the first experiment owed to the chapter. Every method runs on identical splits, multi-seed with error bars, under the Goal G4 protocol. Two notes on how to read it. The Concrete column runs **every** arm on the log-and-standardized features of §4.3, so nothing in it is comparing models fitted on different inputs — an earlier version of this table paired the MoG's best accuracy with a training time measured at raw features, which is exactly the quiet mismatch the harness exists to prevent. Applying the transform uniformly costs the baselines nothing: CART and Random Forest split on rank and are therefore invariant to any monotone transform, which Table 4.1 measures directly at +0.001 and +0.000, so their column reads the same either way and the uniformity is free. And the MoG appears on two rows because only the first is fully instrumented: the 1st-order row's accuracy and training time now genuinely do come from one measured run — `reproduce/outputs/full-14900hx-r2/table_4_1.csv`, independently reproduced by `outputs/table45-full2nd/` — whereas an earlier version of this row paired an R² from that run with a training time from `seeds10-2026-08-01`, where the same arm scored 0.651, and quoted a PhiUSIIL time (0.72 ± 0.04 s) that appears in no archive at all. The full-second-order arm's time is discussed below rather than borrowed from anywhere. The Concrete timing used to read 1.04 ± 0.62 s, and that ±60% was not seed spread: Concrete is the first arm the process fits, so seed 0 absorbed import, JIT, thread-pool spin-up and first-touch allocation, coming in at 3.7× the mean of the other nine. The generator now performs one discarded warm-up fit, and the cell reads **0.84 ± 0.01 s** — a spread of 1%, in line with the PhiUSIIL row, which was never affected because it is fitted second. That asymmetry between two rows of one table is what gave the diagnosis away. Every accuracy in the table is byte-identical across the change, so the warm-up moved the clock and nothing else; see note 14 in `PROVENANCE_MAP.md`.

| Method | Concrete R² | Concrete train time | PhiUSIIL accuracy | PhiUSIIL train time |
|---|---:|---:|---:|---:|
| **MoG FIS (this work)**, 1st order | 0.780 ± 0.029 | **0.84 ± 0.01 s** | **0.997 ± 0.001** | **0.65 ± 0.02 s** |
| **MoG FIS**, full 2nd order | **0.859 ± 0.039** | *no comparable timing exists — see below* | — | — |
...
*The full-second-order row's missing time, and why a plausible number does not go there.* This cell was `*pending*` and is now marked instead, because trying to fill it turned up something worth stating. The two halves of that row do not come from the same code path. Its **R² of 0.859 ± 0.039 comes from Table 4.1's study** (`table_hyperparam_normalization.py`), which drives `solve_tsk_consequents` / `predict_tsk` directly and standardizes the target; row 1's numbers come from `table_4_1_mog_baselines.py` driving the `MixtureOfGaussiansFuzzyRegressor` estimator. `table_4_1_mog_baselines.py` now carries a timed full-second-order arm, and at ten seeds the **estimator** path measures **R² = 0.840 ± 0.049 in 0.83 ± 0.01 s** (`reproduce/outputs/table45-full2nd/`). Those two halves are consistent with each other; neither is consistent with the 0.859 above it. Putting 0.83 s next to 0.859 would pair a time from one implementation with an accuracy from another, which is precisely the quiet mismatch this table's caption says it exists to prevent — the same error, one generator over, as the earlier version that paired a normalized accuracy with a raw-feature time. The cell therefore stays empty until the pipeline that produced 0.859 is itself timed, and the honest reading of the row today is: full second order buys about +0.06 R² over first order and costs no more time than first order does, with the exact pairing owed.
