...
| **9** | **Re-run the non-`tribble-fis` scripts** (Table 3.1 pqVAT under `tribble-cluster`, the gated-minimax pipeline, the `tribble-opt` TSP sample) to confirm they are untouched, and record that they are. | — | 1 h |
