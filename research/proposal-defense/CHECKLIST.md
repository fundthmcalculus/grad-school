...
- [ ] 🟨 **D9 — Advisor request (2026-09-03): more analytical derivations, many more figures and diagrams.**
      ...
      **(c) Figures, Chapters 5–6 — done in PR 3.** Seven new generators: 5.4 the persistence gate deciding (persistence diagram with the MAD gate, and the cover it yields, on rings and bridge), 5.5 the five battery datasets coloured by the set-cover's blocks, 5.6 Table 5.4's scaling sweep (`full-2026-08-22`) with the granularity-agreement fractions; 6.1 the shared ridge solver (a real $\Phi$ plus the three-consumer diagram), 6.3 soft-tree routing weights as a partition of unity, 6.5 the Ruspini export and apex-only refinement, 6.6 the refinement ladder (`uniform-2026-08-03`). Chapter 5's Figures 5.4–5.6 run the `gated-minimax-selection` code through the same calls as Table 5.1. Renumbered: 6.1→6.2, 6.2→6.4. Total figures 16 → 35. Figure numbering is by order of appearance, so PRs 2
