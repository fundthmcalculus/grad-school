...
def three_level_hierarchy(
    n_per=12, seed=11, l1_sep=200.0, l2_sep=28.0, l3_sep=5.0, sigma=0.35
):
...
def density_hierarchy(n_per=25, seed=19, super_sep=30.0, sub_sep=5.0):
    ...
    sub_layout = [(np.array([0.0, 0.0]), 0.30), (np.array([sub_sep, 0.0]), 0.75)]
    X, y_fine, y_coarse = [], [], []
    fine = 0
    for si, sc in enumerate(supers):
...
def scalable_log_separated(
    n, seed=33, decades=(0, 1.3, 2.6), base_sigma=0.35, sep_factor=7.0
):
