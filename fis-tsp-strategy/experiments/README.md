...
| `transfer.py` | **reported** (FINDINGS §4.2) | Fits a rule base on one structural family and tests it on all four plus held-out TSPLIB, producing the transfer matrix that turns "it works on the test set" into "it generalises". Lives here rather than in the pipeline only because it re-fits from scratch and so is not something `run_all.py` should do on every run; its result is a reported finding. Reuses `tune_opt.run_one` rather than reimplementing the fitting. |
| `profile_kernels.py` | **diagnostic** | Where the solver's time goes, three ways that check each other: counter attribution through the fitted cost model, microbenchmarks with the loop inside nopython mode, and ablation. Its findings are written up in FINDINGS §8. Re-run after any change to the hot path, or on new hardware — the cost model's calibration is machine-specific and the profile's *shares* are the durable part, not its totals. |
...
`kick.py --instance … --kicks …` is the plateau probe of FINDINGS §6.1: it carries the
