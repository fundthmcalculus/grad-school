...
def _stages(args):
    ...
    return [
        (
            "tests",
            [PY, "test_invariants.py"],
            "nothing — it either passes or stops the run",
        ),
        ("costmodel", [PY, "costmodel.py"], paths.COSTMODEL),
        (
            "tune-small",
            [PY, "tune_opt.py", "--scale", "small", *quick],
            paths.tuned("small"),
        ),
        (
            "tune-large",
            [PY, "tune_opt.py", "--scale", "large", *quick],
            paths.tuned("large"),
        ),
        (
            "bench-small",
            [PY, "benchmark.py", "--scale", "small", *reps],
            paths.benchmark("small"),
        ),
        (
            "bench-large",
            [PY, "benchmark.py", "--scale", "large", *reps],
        ...
        (
            "figs-tuning",
            [PY, "figures_tuning.py"],
            paths.FIGURES / "fis_tsp_tuning.png",
        ),
...
def main():
    ...
    chosen = [
        s
        for s in stages
        if (args.stages is None or s[0] in args.stages) and s[0] not in args.skip
    ]
    if args.stages:
