...
def main():
    ...
    if b_optimize_coeff:
        y_bucket_mean_opt_0 = optimize_tsk_coefficients(
            X_train,
    ...
    for ij, y_id in enumerate(labels):
        ...
        y_test_pred_second_order_full_opt[:] += (
            y_bucket_mean_opt_2f[y_id] + X_test_rule2f @ corr_terms_2f_opt[y_id, :]
        ) * norm_firing_strength[:, ij]
    ...
    r2_2fopt, rmse_2fopt = report_regression_performance(
        start_time, y_test, y_test_pred_second_order_opt, n_order="2-full optimized"
    )
    plot_tsk_order_comparison(
        [r2_0opt, r2_1opt, r2_2opt, r2_2fopt],
        [rmse_0opt, rmse_1opt, rmse_2opt, rmse_2fopt],
        y_test,
        [
        ...
        ["0 Optimized", "1 Optimized", "2 Optimized", "2-full Optimized"],
    )
