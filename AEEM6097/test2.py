...
def main():
    ...
    print(
        f"Recursive best path: {np.array(recur_best_path)+1}. Tour length={recur_best_len}"
    )
    ...
    with Parallel(n_jobs=joblib.cpu_count() // 2) as parallel:
        for generation in tqdm.trange(n_iter, desc="ACO Generation"):
            ...
            all_results = parallel(
                delayed(parallel_ant)(i_ant) for i_ant in range(n_ants)
            )
...
def recursive_find_best_soln(
    network_routes, cur_city=0, used_cities=None, cur_length=0
    ...
    for i in range(network_routes.shape[0]):
        ...
        if network_routes[cur_city, i] == -1:
            continue
        yield from recursive_find_best_soln(
            network_routes, i, used_cities.copy(), cur_length
        )
