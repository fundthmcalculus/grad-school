...
class VariablesInfo:
    ...
    ) -> tuple[int, AcoContinuousVariable | None]:
    ...
    def append_variables(
        self, x: AcoContinuousVariable | list[AcoContinuousVariable]
    ) -> None:
...
class FuzzyDataSet:
    ...
    def create_from_data(
        data: af64, test_percent: f64, labels: list[str]
    ) -> "FuzzyDataSet":
    ...
    def scale_data(self, data: af64) -> af64:
        return (data - self.scale_info.data_min) / (
            self.scale_info.data_max - self.scale_info.data_min
        )

    def unscale_data(self, data: af64, index: int | None = None) -> af64:
        if index is None:
            return (
                data * (self.scale_info.data_max - self.scale_info.data_min)
                + self.scale_info.data_min
            )
        else:
            return (
                data
                * (self.scale_info.data_max[index] - self.scale_info.data_min[index])
                + self.scale_info.data_min[index]
            )
