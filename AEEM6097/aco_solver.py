...
def get_truncated_normal(mean=0, stdev=1, low=0, high=10):
    return truncnorm(
        (low - mean) / stdev, (high - mean) / stdev, loc=mean, scale=stdev).rvs()
...
class AcoVariable:
    ...
    @abstractmethod
    def random_value(self, rng: Generator | None = None, current_value: np.float64 = np.nan, other_values: np.array = None, learning_rate: float = 0.7) -> np.float64:
        pass

    @abstractmethod
    def initial_random_value(self, rng: Generator | None = None, perturbation: float = 0.1) -> np.float64:
        pass
...
class AcoDiscreteVariable(AcoVariable):
    ...
    def random_value(self, rng: Generator = None, current_value: np.float64 = np.nan, other_values: np.array = None, learning_rate: float = 0.7):
        if rng is None:
...
class AcoContinuousVariable(AcoVariable):
    def __init__(self, name: str, lower_bound: float, upper_bound: float, initial_value: float = None, perturbation: float = 0.1):
        super().__init__(name)
        self.lower_bound = lower_bound
        self.upper_bound = upper_bound
        self.initial_value = self.random_value()
    ...
    def random_value(self, rng: Generator = None, current_value: np.float64 = np.nan, other_values: np.array = None, learning_rate: float = 0.7):
        ...
        if other_values is not None:
            ...
            return get_truncated_normal(
                mean=current_value, stdev=learning_rate*D2,
    ...
    def initial_random_value(self, rng: Generator | None = None, perturbation: float = 0.1) -> np.float64:
        if rng is None:
            rng = np.random.default_rng()
        return max(self.lower_bound, min(self.upper_bound, self.initial_value * (1 + rng.uniform(-perturbation, perturbation))))
...
def solve_aco(fcn: Callable[[np.array], np.float64],
              variables: list[AcoVariable],
              num_generations: int = 30,
              solution_archive_size: int = 30,
              learning_rate: float = 0.7,
              num_ants: int = 20,
              q: float = 1.0) -> tuple[np.array, np.array]:
    ...
    solution_weights = 1 / (q * solution_archive_size * np.sqrt(2 * np.pi)) * np.exp(
        -((j - 1 / 2) ** 2) / (2 * (q * solution_archive_size) ** 2)
    )
    ...
    for generation in generation_pbar:
        job_output = joblib.Parallel(n_jobs=n_jobs)(joblib.delayed(run_ants)(ants_per_job, cp_j, fcn, generation, learning_rate, rng, solution_archive, variables) for job in range(n_jobs))
        # for ant in range(num_ants):
        #     run_ant(num_ants/n_jobs, ant_solutions, ant_values, cp_j, fcn, generation, learning_rate, new_solution, rng,
        #             solution_archive, variables)
...
def run_ants(n_ants, cp_j, fcn, generation, learning_rate, rng, solution_archive, variables, step_explore=False):
    ...
    for ant in range(n_ants):
        ...
        for i, variable in enumerate(variables):
            ...
            new_solution[i] = variable.random_value(current_value=base_solution[i], other_values=solution_archive[:, i],
        ...
        if step_explore:
            for j, variable in enumerate(variables):
                if isinstance(variable, AcoContinuousVariable):
                    # Step up or down by a fraction of the total domain - shrink as we get to higher generations
                    dv = rng.uniform(-0.1, 0.1) * (variable.upper_bound - variable.lower_bound) / (1 + generation)
                    new_solution[j] += dv
                    trial_value = fcn(new_solution)
...
if __name__ == "__main__":
    best_soln, soln_history = solve_aco(test_ackley, [AcoContinuousVariable("x", -15, 30), AcoContinuousVariable("y", -15, 30)])
