...
def ga_solve_permutation(
    f: typing.Callable[[np.ndarray], np.float64], x0: np.ndarray
) -> tuple[np.float64, np.ndarray, list[np.float64]]:
...
def plot_ga_results(
    ...
    permuted_distances: np.ndarray,
    optimal_order: np.ndarray,
    soln_history,
):
    ...
    random_heatmap = ax2.imshow(
        distances, cmap="viridis", aspect="auto", interpolation="nearest"
    )
    ...
    random_heatmap = ax3.imshow(
        permuted_distances, cmap="viridis", aspect="auto", interpolation="nearest"
    )
...
def main():
    ...
    print(
        f"Random distance={permuted_distances.diagonal(offset=1).sum():.2f}, "
