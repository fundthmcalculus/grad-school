...
def y_def(x: af64) -> af64:
    ...
    # Ackley function
    return (
        -20.0 * np.exp(-0.2 * np.sqrt(0.5 * x**2))
...
def y_crisp(
    x: af64,
    mu_x: list[Callable[[af64, f64, f64], af64]],
    f_x: list[Callable[[af64, f64, f64], af64]],
    args: af64,
) -> af64:
...
def main_ackley(min_fcn) -> None:
    ...
    for ij, mu_i in enumerate(mu_lst):
        axs[1].plot(
            x_eval, mu_i(x_eval, x1[n_args * ij], x1[n_args * ij + 1]), label=f"mu_{ij}"
        )
    axs[1].legend()
...
def plot_ivat(sonar_full_data):
    ...
    ivat_sonar_data, _, _, _ = compute_ivat_ordered_dissimilarity_matrix2(
        sonar_full_data
    )
    fig = plt.figure()
