...
class AtwoodMachine:
    ...
    def equations_of_motion(self, state, t):
        ...
        r_ddot = (
            r * omega**2 + self.g * np.cos(theta) - self.k / self.m2 * (r - self.r0)
        ) / (self.m1 / self.m2 + 1)
...
def train_and_evaluate_rollout(
    X_train, y_train, initial_states_test, pendulum, n_steps=100, dt=0.01
):
    """
    ...
    for init_state in initial_states_test:
        traj = rollout_predict(
            (regressor_r, regressor_theta), pendulum, init_state, n_steps, dt
        )
...
def plot_rollout_comparison(results_rollout, actual_trajectories, dt=0.01):
    ...
    ax = axes[0, 0]
    for i, (pred_traj, actual_traj) in enumerate(
        zip(results_rollout["rollout_trajectories"], actual_trajectories)
    ):
...
def plot_trace_comparison(results_single, results_window):
    ...
    fig.suptitle(
        "Prediction Traces: Actual vs Predicted", fontsize=16, fontweight="bold"
    )
    ...
    ax.set_title(
        f'Single-Step Predictions (R²={results_single["r2"]:.4f})', fontsize=12
    )
    ax.legend(loc="best")
    ax.grid(True, alpha=0.3)
    ...
    ax.set_ylabel(f"{OUTPUT_FEATURES[0]} (rad)", fontsize=11)
    ax.set_title(
        f'Multi-Step Window Predictions (window=3, R²={results_window["r2"]:.4f})',
        fontsize=12,
    )
    ax.legend(loc="best")
    ax.grid(True, alpha=0.3)
...
class TestAtwoodMachineFuzzyPrediction(unittest.TestCase):
    ...
    def test_double_pendulum_fuzzy_prediction(self):
        ...
        generate_simulation_data(data_dir, num_simulations=15, duration=3.0, dt=0.01)
        ...
        print("#" * 60)
        window_size = 3
        X_window_train, y_window_train = load_and_prepare_data(
            data_dir,
            input_features=INPUT_FEATURES,
            output_features=OUTPUT_FEATURES,
            window_size=window_size,
        )
        X_window_test, y_window_test = load_and_prepare_data(
            data_dir,
            file_glob="simulation_tst*.csv",
            input_features=INPUT_FEATURES,
            output_features=OUTPUT_FEATURES,
        ...
        results_window = train_and_evaluate_window(
            N_BINS,
            OUTPUT_FEATURES,
            X_window_train,
            y_window_train,
        ...
        print("#" * 60)
        X_rollout_train, y_rollout_train = load_and_prepare_data(
            data_dir,
            input_features=INPUT_FEATURES,
            output_features=OUTPUT_FEATURES,
            window_size=1,
        ...
        for test_file in test_files:
            ...
            initial_states_test.append(
                df[["r", "theta", "r_dot", "omega"]].iloc[0].values
            )
        ...
        for i, (pred_traj, actual_traj) in enumerate(
            zip(results_rollout["rollout_trajectories"], actual_test_trajectories)
        ):
