...
from ode_helpers import load_and_prepare_data, train_and_evaluate_single_step, find_nearest_trajectories
...
def generate_family(n, base_thetas_deg, sweep_index, sweep_deltas_deg, test_delta_deg,
                     m_vals=None, l_vals=None, g=9.81, dt=0.01, duration=30.0) -> NPendulumFamily:
    ...
    return NPendulumFamily(pendulum=pendulum, train_trajectories=train_trajectories,
                            test_trajectory=test_trajectory, base_thetas_deg=base_thetas_deg,
                            sweep_index=sweep_index, dt=dt, duration=duration)
...
def check_family_energy(family: NPendulumFamily):
    ...
    print(f"  Family energy check: max drift across all trajectories = {max_pct:.5f}% of |E0|")
    return max_pct
...
def mimo_feature_steps(window_size, feature_names):
    ...
    return [(f"{feat}_step{i}", i, feat) for i in range(window_size) for feat in feature_names]
...
def train_mimo(feature_names, X_train, y_train, X_test, y_test, window_size, n_bins=3):
    ...
    regressor = MimoGaussianPredictor(n_output_buckets=n_bins, tsk_order="1st",
    ...
    for col in feature_names:
        ...
        metrics[col] = dict(mse=mse, rmse=np.sqrt(mse),
                             mae=mean_absolute_error(y_test_df[col], y_pred_df[col]),
                             r2=r2_score(y_test_df[col], y_pred_df[col]))
    return dict(regressor=regressor, metrics=metrics, y_test=y_test_df, y_pred=y_pred_df,
                window_size=window_size, input_cols=input_cols)


def run_iterative_prediction(regressor, initial_window_df, feature_names, n_steps, window_size=1):
    ...
    for step in range(n_steps):
        if diverged_at:
            running_state = pd.concat(
                [running_state, pd.DataFrame([np.full(running_state.shape[1], np.nan)],
        ...
        if window_size == 1:
            ...
            for step_name, offset, col in steps:
                if offset < window_size - 1:
                    ...
                    new_row[step_name] = running_state.iloc[-1][most_recent] + next_delta_df.iloc[0][col]
...
def render_comparison_gif(family: NPendulumFamily, predicted_theta_df, out_path,
                           max_frames=320, fps=28, trail_len=60, figsize=(11, 6), dpi=110):
    p = family.pendulum
    n = p.n
    feature_names = [f'theta_{i}' for i in range(1, n + 1)]
    actual_theta = family.test_trajectory[feature_names].iloc[:len(predicted_theta_df)].values
    ...
    fig.suptitle(f'{n}-Link Pendulum: Actual vs. Fuzzy-TSK Prediction', color='white', fontsize=14, fontweight='bold')
    time_text = fig.text(0.5, 0.02, '', ha='center', color='#aaaaaa', fontsize=10)

    trail_a, = axes[0].plot([], [], '-', color='#00d4ff', linewidth=1.1, alpha=0.55)
    chain_a, = axes[0].plot([], [], 'o-', color='#e0e0e0', lw=2.2, ms=5, markerfacecolor='white')
    bob_a, = axes[0].plot([], [], 'o', color='#00d4ff', ms=8)

    trail_p, = axes[1].plot([], [], '-', color='#ff6b6b', linewidth=1.1, alpha=0.55)
    chain_p, = axes[1].plot([], [], 'o-', color='#e0e0e0', lw=2.2, ms=5, markerfacecolor='white')
    ...
    def update(frame):
        ...
        trail_a.set_data(x_act[t_start:i + 1:max(1, step // 2 + 1), -1],
        ...
        trail_p.set_data(x_pred[t_start:i + 1:max(1, step // 2 + 1), -1],
    ...
    plt.close(fig)
    print(f"  Saved {out_path.name}: {n_frames} frames, {out_path.stat().st_size / 1e6:.2f} MB")


def run_study(n, base_thetas_deg, sweep_index, label, out_prefix, m_vals=None, l_vals=None,
              g=9.81, dt=0.01, duration=30.0, window_sizes=(1, 3), n_bins=3):
    print("=" * 70)
    ...
    family = generate_family(n, base_thetas_deg, sweep_index, sweep_deltas, test_delta_deg=2.05,
                              m_vals=m_vals, l_vals=l_vals, g=g, dt=dt, duration=duration)
    print(f"Generated {len(family.train_trajectories)} training trajectories "
    ...
    X_tr, y_tr = load_and_prepare_data(family.train_trajectories, feature_names, feature_names, window_size=1)
    X_te, y_te = load_and_prepare_data([family.test_trajectory], feature_names, feature_names, window_size=1)
    single_step = train_and_evaluate_single_step(n_bins, feature_names, X_tr, y_tr, X_te, y_te)

    mimo_results = {}
    for ws in window_sizes:
        print(f"\nMIMO model, window={ws}...")
        Xw_tr, yw_tr = load_mimo_data(family.train_trajectories, feature_names, window_size=ws)
        Xw_te, yw_te = load_mimo_data([family.test_trajectory], feature_names, window_size=ws)
        t1 = time.perf_counter()
        res = train_mimo(feature_names, Xw_tr, yw_tr, Xw_te, yw_te, window_size=ws, n_bins=n_bins)
    ...
    best_ws = min(window_sizes)  # window=1 for iterative-rollout comparability with the n=2 study
    ...
    predicted = run_iterative_prediction(mimo_results[best_ws]['regressor'], seed,
                                          feature_names, n_steps, window_size=best_ws)

    valid_mask = ~predicted[feature_names[0]].isna()
    n_valid = valid_mask.sum()
    print(f"  Valid rollout steps: {n_valid}/{len(predicted)} "
          f"({n_valid * dt:.2f}s / {duration:.2f}s)")
    if n_valid > 2:
        for col in feature_names:
            act = family.test_trajectory[col].values[:len(predicted)][valid_mask.values]
            pred = predicted[col].values[valid_mask.values]
            print(f"  {col}: MAE={np.mean(np.abs(act - pred)):.4f}  R2={r2_score(act, pred):.4f}")

    print("\nRendering actual-vs-predicted GIF...")
    render_comparison_gif(family, predicted, FIG_DIR.parent / f"{out_prefix}_fuzzy_comparison.gif")

    print("\nPlotting nearest-training-trajectory comparison...")
    nearest = find_nearest_trajectories(family.test_trajectory, family.train_trajectories, k=2,
    ...
    for idx, col in enumerate(feature_names):
        ax = axes[idx]
        ax.plot(t_test, family.test_trajectory[col].values, color='#00d4ff', lw=2, label='Test (reference)', zorder=5)
        t_pred = np.arange(len(predicted)) * dt
        ax.plot(t_pred[valid_mask], predicted[col].values[valid_mask], color='#ff1744', lw=1.6,
                label='Test (predicted)', alpha=0.85, zorder=4)
        for rank, (tr_idx, dist, tr_df) in enumerate(nearest):
            t_tr = np.arange(len(tr_df)) * dt
            ax.plot(t_tr, tr_df[col].values, '--', color=['#00DD00', '#00AA00'][rank], lw=1.2, alpha=0.6,
                    label=f'Train {tr_idx} (d={dist:.3f})')
        ax.set_title(col, fontsize=11)
        ax.set_xlabel('Time (s)')
        ax.grid(alpha=0.3)
        ax.legend(fontsize=7, loc='best')
    fig.suptitle(f'n={n} ({label}): Test vs. Predicted vs. Nearest Training', fontsize=13, fontweight='bold')
    plt.tight_layout()
    fig.savefig(FIG_DIR / f"{out_prefix}_nearest_training.png", dpi=180, bbox_inches='tight')
    plt.close(fig)

    return dict(family=family, single_step=single_step, mimo=mimo_results, predicted=predicted,
                n_valid=n_valid)
...
if __name__ == '__main__':
    print("\n\n### n=3 STUDY ###\n")
    result3 = run_study(
        n=3, base_thetas_deg=[120.0, 60.0, 0.0], sweep_index=2,
        label="fan configuration, sweeping theta_3", out_prefix="n3",
    )

    print("\n\n### n=5 STUDY (inverted zigzag) ###\n")
    result5 = run_study(
        n=5, base_thetas_deg=[170.0, -170.0, 170.0, -170.0, 170.0], sweep_index=4,
        label="inverted zigzag, sweeping theta_5", out_prefix="n5",
    )
