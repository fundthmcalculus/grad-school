...
class DataSimulation:
    ...
    def get_combined_data(self: "DataSimulation") -> tuple[DataFrame, DataFrame]:
        ...
        X_combined = pd.concat(
            [t.iloc[:-1] for t in self.trajectories], ignore_index=True
        )
        # 1st order differentiation
        y_combined = pd.concat(
            [t.diff().iloc[1:] / self.params.dt for t in self.trajectories],
            ignore_index=True,
        )
...
def test_tribble_ode():
    ...
    ax_phase.set_title(
        "Phase Space (θ₂ vs ω₂)", color="white", fontsize=11, fontweight="bold"
    )
    ...
    (phase_act,) = ax_phase.plot(
        [], [], color="#00d4ff", linewidth=1.5, label="Actual", marker="o", markersize=3
    )
    (phase_pred,) = ax_phase.plot(
        [],
        [],
        color="#ff6b6b",
        linewidth=1.5,
    ...
    def update(frame):
        ...
        x1_act, y1_act, x2_act, y2_act = angles_to_xy(
            theta1_act, theta2_act, test_results.model.l1, test_results.model.l2
        )
        ...
        x1_pred, y1_pred, x2_pred, y2_pred = angles_to_xy(
            theta1_pred, theta2_pred, test_results.model.l1, test_results.model.l2
        )
        ...
        phase_pred.set_data(
            predicted.iloc[: idx + 1, 2].values, predicted.iloc[: idx + 1, 3].values
        )
