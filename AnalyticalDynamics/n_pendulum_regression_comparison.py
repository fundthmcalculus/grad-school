...
if __name__ == '__main__':
    ...
    t2, err2 = rollout_error(['theta_1', 'theta_2'], train2.trajectories, test2.trajectories[0], dt)
    results[2] = (t2, err2)

    print("n=3 (fan configuration)...")
    fam3 = generate_family(3, [120.0, 60.0, 0.0], 2, np.arange(1.5, 3.00001, 0.1), test_delta_deg=2.05,
                            dt=dt, duration=30.0)
    t3, err3 = rollout_error(['theta_1', 'theta_2', 'theta_3'], fam3.train_trajectories, fam3.test_trajectory, dt)
    results[3] = (t3, err3)

    print("n=5 (inverted zigzag)...")
    fam5 = generate_family(5, [170.0, -170.0, 170.0, -170.0, 170.0], 4, np.arange(1.5, 3.00001, 0.1),
                            test_delta_deg=2.05, dt=dt, duration=30.0)
    t5, err5 = rollout_error([f'theta_{i}' for i in range(1, 6)], fam5.train_trajectories, fam5.test_trajectory, dt)
    results[5] = (t5, err5)
    ...
    ax.set_title('Open-Loop Rollout Error Growth vs. Chain Length\n(fuzzy TSK, window=1, corrected rollout)')
    ax.legend()
