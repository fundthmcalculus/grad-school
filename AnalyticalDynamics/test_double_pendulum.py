...
def mimo_input_feature_names(
    window_size: int, feature_names: list[str] | None = None
) -> list[FeatureStep]:
    ...
    if window_size == 1:
        return [
            FeatureStep(step_name=feat, step_offset=0, col_name=feat)
...
def load_and_prepare_mimo_data(trajectories: list[pd.DataFrame], window_size: int = 1):
    ...
    """
    print(
        f"Loading {len(trajectories)} simulation files for MIMO (window={window_size})..."
    )
...
def get_mimo_df(df: pd.DataFrame, window_size: int) -> pd.DataFrame:
    ...
    for f_step in feature_steps:
        X[f_step.step_name] = (
            df[f_step.col_name]
            .iloc[f_step.step_offset : -(window_size - f_step.step_offset)]
            .values
        )
    return X
...
def run_iterative_prediction(
    regressor,
    initial_window_df: pd.DataFrame,
    n_steps,
    window_size: int = 1,
    verbose: bool = True,
):
    ...
    if len(initial_window_df) < window_size:
        raise ArgumentError(
            "Initial data must be greater than or equal to window size."
        )
        ...
        if diverged_at:
            ...
            if window_size == 1:
                ...
                for fs in feature_steps:
                    if fs.step_offset < window_size - 1:
                        ...
                        new_row[fs.step_name] = (
                            running_state.iloc[-1][most_recent_col]
                            + next_state_delta_df.iloc[0][fs.col_name]
                        )
            ...
            if np.any(np.isnan(new_state)) or np.any(np.abs(new_state) > 1e4):
                if diverged_at is None:
                    ...
                    if verbose:
                        print(
                            f"  Warning: prediction diverged at step {diverged_at} -- padding remainder with NaN."
                        )
            else:
...
def create_pendulum_animation(
    actual_df, predicted_df, output_path, dt=0.01, max_frames=500, fps=30
):
    """
    ...
    axes[1].set_title(
        "Predicted (iterative rollout)", color="white", fontsize=13, fontweight="bold"
    )
    fig.suptitle(
        "Double Pendulum: Actual vs Predicted",
        color="white",
        fontsize=14,
        fontweight="bold",
    )
    ...
    (act_rod1,) = axes[0].plot(
        [], [], "o-", color="#e0e0e0", lw=2.5, ms=6, markerfacecolor="white"
    )
    (act_rod2,) = axes[0].plot(
        [], [], "o-", color="#e0e0e0", lw=2.5, ms=6, markerfacecolor="#00d4ff"
    )

    # Predicted panel objects
    (pred_trail2,) = axes[1].plot(
        [], [], "-", color="#ff6b6b", linewidth=1.2, alpha=0.5
    )
    (pred_rod1,) = axes[1].plot(
        [], [], "o-", color="#e0e0e0", lw=2.5, ms=6, markerfacecolor="white"
    )
    (pred_rod2,) = axes[1].plot(
        [], [], "o-", color="#e0e0e0", lw=2.5, ms=6, markerfacecolor="#ff6b6b"
    )
...
def create_pendulum_animation_with_training(
    actual_df,
    predicted_df,
    train_trajectories,
    output_path,
    ...
    nearest_list = find_nearest_trajectories(
        actual_df, train_trajectories, k=2, features=OUTPUT_FEATURES
    )
    ...
    ax.set_title(
        "Double Pendulum: All Trajectories Overlaid",
        color="white",
        fontsize=14,
        fontweight="bold",
    )
    fig.suptitle(
        f"Training (dashed) · Test Reference (solid cyan) · Test FIS (solid red)",
        color="white",
        fontsize=12,
    )
    ...
    artists = {
        "train1": {
            "trail2": ax.plot(
                [], [], "--", color=colors_train[0], linewidth=1.5, alpha=0.6
            )[0],
            "rod1": ax.plot(
                [],
                [],
                "o--",
                color=colors_train[0],
                lw=1.5,
                ms=4,
                markerfacecolor=colors_train[0],
                alpha=0.5,
            )[0],
            "rod2": ax.plot(
                [],
        ...
        "train2": {
            "trail2": ax.plot(
                [], [], "--", color=colors_train[1], linewidth=1.5, alpha=0.6
            )[0],
            "rod1": ax.plot(
                [],
                [],
                "o--",
                color=colors_train[1],
                lw=1.5,
                ms=4,
                markerfacecolor=colors_train[1],
                alpha=0.5,
            )[0],
            "rod2": ax.plot(
                [],
        ...
        "actual": {
            "trail2": ax.plot([], [], "-", color="#00d4ff", linewidth=2.0, alpha=0.9)[
                0
            ],
            "rod1": ax.plot(
                [], [], "o-", color="#e0e0e0", lw=2.5, ms=6, markerfacecolor="white"
        ...
        "predicted": {
            "trail2": ax.plot([], [], "-", color="#ff1744", linewidth=2.0, alpha=0.9)[
                0
            ],
            "rod1": ax.plot(
                [], [], "o-", color="#e0e0e0", lw=2.5, ms=6, markerfacecolor="white"
            ...
            "rod2": ax.plot(
                [], [], "o-", color="#e0e0e0", lw=2.5, ms=6, markerfacecolor="#ff1744"
            )[0],
        },
    }
    ...
    legend_text = fig.text(
        0.5,
        0.05,
        "Train (green dashed) · Test Ref (cyan solid, tail=35) · Test FIS (red solid, tail=50)",
        ha="center",
        color="#aaaaaa",
        fontsize=10,
    )
    ...
    def update(frame):
        ...
        if i < len(nearest_data[0]["x2"]):
            artists["train1"]["trail2"].set_data(
                nearest_data[0]["x2"][t_start : i + 1],
                nearest_data[0]["y2"][t_start : i + 1],
            )
            artists["train1"]["rod1"].set_data(
                [0, nearest_data[0]["x1"][i]], [0, nearest_data[0]["y1"][i]]
            )
            artists["train1"]["rod2"].set_data(
                [nearest_data[0]["x1"][i], nearest_data[0]["x2"][i]],
                [nearest_data[0]["y1"][i], nearest_data[0]["y2"][i]],
            )

        # Training 2 (background) - medium tail
        t_start = max(0, i - trail_lens["train2"])
        if i < len(nearest_data[1]["x2"]):
            artists["train2"]["trail2"].set_data(
                nearest_data[1]["x2"][t_start : i + 1],
                nearest_data[1]["y2"][t_start : i + 1],
            )
            artists["train2"]["rod1"].set_data(
                [0, nearest_data[1]["x1"][i]], [0, nearest_data[1]["y1"][i]]
            )
            artists["train2"]["rod2"].set_data(
                [nearest_data[1]["x1"][i], nearest_data[1]["x2"][i]],
                [nearest_data[1]["y1"][i], nearest_data[1]["y2"][i]],
            )

        # Actual (test reference) - longer tail
        t_start = max(0, i - trail_lens["actual"])
        artists["actual"]["trail2"].set_data(
            x2_act[t_start : i + 1], y2_act[t_start : i + 1]
        )
        artists["actual"]["rod1"].set_data([0, x1_act[i]], [0, y1_act[i]])
        artists["actual"]["rod2"].set_data(
            [x1_act[i], x2_act[i]], [y1_act[i], y2_act[i]]
        )

        # Predicted (front) - longest tail
        t_start = max(0, i - trail_lens["predicted"])
        artists["predicted"]["trail2"].set_data(
            x2_pred[t_start : i + 1], y2_pred[t_start : i + 1]
        )
        artists["predicted"]["rod1"].set_data([0, x1_pred[i]], [0, y1_pred[i]])
        artists["predicted"]["rod2"].set_data(
            [x1_pred[i], x2_pred[i]], [y1_pred[i], y2_pred[i]]
        )
...
def plot_mimo_state_trajectories(actual_df, predicted_df, dt=0.01):
    ...
    axes_flat = axes.flat
    fig.suptitle(
        "MIMO Iterative Rollout: Actual vs Predicted", fontsize=14, fontweight="bold"
    )
...
class TestDoublePendulumFuzzyPrediction(unittest.TestCase):
    ...
    def test_double_pendulum_fuzzy_prediction(self):
        ...
        with time_this("train-single"):
            X_single_train, y_single_train = load_and_prepare_data(
                train_results.trajectories,
                INPUT_FEATURES,
                OUTPUT_FEATURES,
                window_size=1,
            )
            X_single_test, y_single_test = load_and_prepare_data(
                test_results.trajectories,
                INPUT_FEATURES,
                OUTPUT_FEATURES,
                window_size=1,
            )
            results_single = train_and_evaluate_single_step(
                N_BINS,
                OUTPUT_FEATURES,
                X_single_train,
                y_single_train,
        ...
        with time_this("train-mimo-window1"):
            X_mimo_train, y_mimo_train = load_and_prepare_mimo_data(
                train_results.trajectories, window_size=MIMO_WINDOW_SIZE
            )
            X_mimo_test, y_mimo_test = load_and_prepare_mimo_data(
                test_results.trajectories, window_size=MIMO_WINDOW_SIZE
            )
        ...
        with time_this("train-mimo-window3"):
            X_mimo_train_w3, y_mimo_train_w3 = load_and_prepare_mimo_data(
                train_results.trajectories, window_size=3
            )
            X_mimo_test_w3, y_mimo_test_w3 = load_and_prepare_mimo_data(
                test_results.trajectories, window_size=3
            )
        ...
        print("#" * 60)
        print(
            "Training MIMO models with different memory windows to assess stability..."
        )
        ...
        for ws in window_sizes:
            ...
            with time_this(f"train-mimo-window{ws}"):
                X_train_ws, y_train_ws = load_and_prepare_mimo_data(
                    train_results.trajectories, window_size=ws
                )
                X_test_ws, y_test_ws = load_and_prepare_mimo_data(
                    test_results.trajectories, window_size=ws
                )
        ...
        print("\n" + "#" * 60)
        print(
            "# STEP 5: Iterative Rollout from Initial Conditions (Multiple Window Sizes)"
        )
        print("#" * 60)
        ...
        for ws in window_sizes:
            ...
            with time_this(f"iterative-rollout-window{ws}"):
                predicted_traj = run_iterative_prediction(
                    mimo_results_by_window[ws]["regressor"],
                    seed_df,
                    n_steps,
                    window_size=ws,
                    verbose=False,
            ...
            print(
                f"  Window {ws}: Diverged at step {divergence_idx} ({divergence_time:.2f}s) / {len(tst_df)} total steps"
            )

        # Use default window size for detailed analysis
        predicted_trajectory = predicted_trajectories_by_window[MIMO_WINDOW_SIZE]
        actual_trajectory = (
            tst_df[OUTPUT_FEATURES].iloc[MIMO_WINDOW_SIZE - 1 :].reset_index(drop=True)
        )
        ...
        def rollout_metrics(label, predicted, actual, n):
            ...
            for col in OUTPUT_FEATURES:
                ...
                print(
                    f"  {col:10s}: MAE={mae:.4f}  R2={r2:.4f}  (valid={valid.sum()}/{n})"
                )

        rollout_metrics(
            "Iterative Rollout Metrics (window=1)",
        ...
        for summary in stability_summary:
            print(
                f"{summary['window_size']:<15} {summary['divergence_step']:<20} {summary['divergence_time_s']:<25.2f} {summary['trajectory_length']*SimulationParams.dt:<20.2f}"
            )

        best_window = max(stability_summary, key=lambda x: x["divergence_time_s"])
        print("\n" + "=" * 70)
        print(
            f"Best Stability: Window size {best_window['window_size']} stays stable for {best_window['divergence_time_s']:.2f}s"
        )
        print("=" * 70)
        ...
        fig3 = plot_mimo_state_trajectories(
            actual_trajectory, predicted_trajectory, dt=SimulationParams.dt
        )
        ...
        fig_nearest.savefig(
            "test_vs_nearest_training.png", dpi=200, bbox_inches="tight"
        )
