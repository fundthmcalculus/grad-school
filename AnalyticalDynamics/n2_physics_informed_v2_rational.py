...
RATIONAL_BASIS_COLS = [
    'sin_th1_over_d1', 'sin_th1m2th2_over_d1', 'sinD_om2sq_over_d1', 'sinD_om1sq_cosD_over_d1',
    'sinD_om1sq_over_d2', 'sinD_costh1_over_d2', 'sinD_om2sq_cosD_over_d2',
]


def rational_basis_features(df: pd.DataFrame, l1: float, l2: float, m1: float, m2: float) -> pd.DataFrame:
...
ALPHA1_COLS = ['sin_th1_over_d1', 'sin_th1m2th2_over_d1', 'sinD_om2sq_over_d1', 'sinD_om1sq_cosD_over_d1']
...
if __name__ == '__main__':
    ...
    for col in OMEGA_COLS:
        ...
        print(f"  {col}: R2={r2_score(y_test_df[col], y_pred_df[col]):.6f}  "
    ...
    if valid.sum() > 2:
        for col in STATE_COLS:
            ...
            print(f"  {col}: MAE={np.mean(np.abs(a - p)):.6f}  R2={r2_score(a, p):.6f}  "
    ...
    axes[0].plot(t[valid.values], predicted['theta_1'].values[valid.values], color='#ff1744', lw=1.6,
                 label='Predicted (rational physics basis)', alpha=0.85)
    ...
    axes[1].semilogy(t, np.maximum(err_theta1, 1e-8), color='#d62728', lw=1.6,
    ...
    fig.suptitle('n=2: Physics-Inspired Consequent, v2 -- Known Rational (Division) Structure', fontweight='bold')
    plt.tight_layout()
    fig.savefig(FIG_DIR / 'n2_physics_informed_v2_rollout.png', dpi=200, bbox_inches='tight')
