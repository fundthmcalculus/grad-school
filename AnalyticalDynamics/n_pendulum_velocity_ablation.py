...
if __name__ == "__main__":
    ...
    t_a, err_a, res_a = rollout_error(
        train_results.trajectories, tst, ["theta_1", "theta_2"]
    )
    print(f"  time to {THRESHOLD} rad: {time_to_threshold(t_a, err_a):.2f}s")

    print("\nAngles + velocities (theta_1, omega_1, theta_2, omega_2)...")
    t_b, err_b, res_b = rollout_error(
        train_results.trajectories, tst, ["theta_1", "omega_1", "theta_2", "omega_2"]
    )
    ...
    ax.semilogy(
        t_a,
        np.maximum(err_a, 1e-6),
        color="#00d4ff",
        lw=1.6,
        label="angles only (theta_1, theta_2)",
    )
    ax.semilogy(
        t_b,
        np.maximum(err_b, 1e-6),
        color="#ffb703",
        lw=1.6,
        label="angles + velocities (theta, omega)",
    )
    ax.axhline(
        THRESHOLD,
        color="#888",
    ...
    plt.tight_layout()
    fig.savefig(
        "figures/rollout_error_velocity_ablation.png", dpi=200, bbox_inches="tight"
    )
