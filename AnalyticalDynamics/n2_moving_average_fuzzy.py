...
from n_pendulum_fuzzy_regression import (
    mimo_feature_steps,
)  # unused here, kept for column-name parity if needed
...
def generate_training_set(
    theta1_deg=120.0,
    theta2_range_deg=(-15.0, 15.0),
    step_deg=0.3,
    test_theta2_deg=7.35,
    dt=0.01,
    duration=30.0,
):
...
def rollout(regressor, test_trajectory, dt, n_steps=None):
    ...
    for step in range(total_steps):
        ...
        if not np.isfinite(list(new_row.values())).all() or any(
            abs(v) > 1e4 for v in new_row.values()
        ):
...
if __name__ == "__main__":
    print(
        "Generating widened n=2 training set (theta_2 in [-15, 15] deg, step 0.3 deg)..."
    )
    t0 = time.perf_counter()
    pendulum, trajectories, test_trajectory, theta2_grid = generate_training_set()
    print(
        f"  {len(trajectories)} training trajectories generated in {time.perf_counter() - t0:.2f}s "
    ...
    actual = (
        test_trajectory[FEATURE_NAMES].iloc[: len(predicted)].reset_index(drop=True)
    )
    ...
    fig.suptitle(
        "n=2: Moving-Average-Feature Fuzzy Regressor, Wider Training Set",
        fontweight="bold",
    )
    plt.tight_layout()
