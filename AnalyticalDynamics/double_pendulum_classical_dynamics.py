...
def total_energy(df, pendulum: DoublePendulum):
    ...
    th1, om1, th2, om2 = (df['theta_1'].values, df['omega_1'].values,
                          df['theta_2'].values, df['omega_2'].values)
    m1, m2, l1, l2, g = pendulum.m1, pendulum.m2, pendulum.l1, pendulum.l2, pendulum.g
    T = (0.5 * m1 * l1**2 * om1**2
         + 0.5 * m2 * (l1**2 * om1**2 + l2**2 * om2**2
...
def plot_energy_conservation(pendulum, dt=0.001, duration=20.0):
    ...
    axes[1].set_title(f'Energy Drift (max |drift| = {np.max(np.abs(drift)):.2e} J) — integrator error, not physics')
    axes[1].grid(alpha=0.3)

    plt.tight_layout()
    fig.savefig(FIG_DIR / 'energy_conservation.png', dpi=200, bbox_inches='tight')
    plt.close(fig)
    print(f"Energy drift: max |dE| = {np.max(np.abs(drift)):.3e} J "
...
def plot_chaos_sensitivity(pendulum, dt=0.01, duration=20.0, delta0=1e-4):
    ...
    sep = np.sqrt((df_a['theta_1'] - df_b['theta_1'])**2 + (df_a['theta_2'] - df_b['theta_2'])**2)
    sep = sep.values
    sep[sep < 1e-14] = 1e-14

    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    axes[0].plot(t, df_a['theta_2'], label=f'$\\theta_2(0)$={base[2]:.4f} rad', color='#00d4ff')
    axes[0].plot(t, df_b['theta_2'], label=f'$\\theta_2(0)$={perturbed[2]:.4f} rad', color='#ff1744', alpha=0.8)
    ...
    if fit_mask.sum() > 10:
        ...
        lyap = p[0]
        axes[1].semilogy(t[fit_mask], np.exp(np.polyval(p, t[fit_mask])), 'k--',
    ...
    plt.close(fig)
    if fit_mask.sum() > 10:
        print(f"Estimated largest Lyapunov exponent: {lyap:.3f} 1/s "
              f"(e-folding time {1/lyap:.2f} s)" if lyap > 0 else "No positive exponent found in fit window.")
    return df_a, df_b


def plot_trajectory_snapshots(pendulum, dt=0.01, duration=8.0):
    ics = [
        (170 * np.pi / 180, 0.0, 0.0 * np.pi / 180, 0.0, '#00d4ff', 'near-inverted (chaotic)'),
        (30 * np.pi / 180, 0.0, 30 * np.pi / 180, 0.0, '#ff9800', 'small-angle (regular)'),
    ]
    fig, axes = plt.subplots(1, 2, figsize=(13, 6.5))
    for ax, (th1, om1, th2, om2, color, label) in zip(axes, ics):
        df = pendulum.simulate((th1, om1, th2, om2), duration=duration, dt=dt)
        x1, y1, x2, y2 = angles_to_xy(df['theta_1'].values, df['theta_2'].values, pendulum.l1, pendulum.l2)
        n = len(x2)
        for i in range(0, n - 1, 3):
            ax.plot(x2[i:i+2], y2[i:i+2], color=color, alpha=0.15 + 0.7 * i / n, lw=1)
        ax.plot([0, x1[-1]], [0, y1[-1]], 'o-', color='#333', lw=2, ms=5)
        ax.plot([x1[-1], x2[-1]], [y1[-1], y2[-1]], 'o-', color='#333', lw=2, ms=5, markerfacecolor=color)
        ax.plot(0, 0, 'ks', ms=8)
        ax.set_title(f'{label}\n$\\theta_1(0)$={th1*180/np.pi:.0f}°, $\\theta_2(0)$={th2*180/np.pi:.0f}°', fontsize=11)
        ax.set_aspect('equal')
        ax.set_xlim(-2.2, 2.2)
        ax.set_ylim(-2.2, 0.5)
        ax.grid(alpha=0.3)
    fig.suptitle(f'Bob-2 Trajectory Over {duration}s (color fades light→dark with time)', fontweight='bold')
    plt.tight_layout()
