...
def total_energy(theta1, theta2, omega1, omega2, m1, m2, l1, l2, g):
    return (kinetic_energy(theta1, theta2, omega1, omega2, m1, m2, l1, l2)
            + potential_energy(theta1, theta2, m1, m2, l1, l2, g))


def energy_correct_rollout(regressor, test_trajectory, params, n_steps=None, correct=True):
    ...
    E0 = total_energy(state0['theta_1'], state0['theta_2'], state0['omega_1'], state0['omega_2'],
                       m1, m2, l1, l2, g)
    ...
    for step in range(total_steps):
        ...
        if correct:
            ...
            T_raw = kinetic_energy(theta1_new, theta2_new, omega1_raw, omega2_raw, m1, m2, l1, l2)
            budget = E0 - V_new
        ...
        state = pd.Series({'theta_1': theta1_new, 'omega_1': omega1_new,
    ...
    if correct and n_clamped:
        print(f"  (energy budget exhausted / clamped to omega=0 on {n_clamped}/{total_steps} steps)")
...
if __name__ == '__main__':
    ...
    print(f"  uncorrected: {ttt_raw:.2f}s" if ttt_raw is not None else "  uncorrected: never")
    print(f"  energy-corrected: {ttt_corr:.2f}s" if ttt_corr is not None else "  energy-corrected: never")

    E0 = total_energy(actual['theta_1'].iloc[0], actual['theta_2'].iloc[0],
                       actual['omega_1'].iloc[0], actual['omega_2'].iloc[0], *params)
    E_actual = total_energy(actual['theta_1'], actual['theta_2'], actual['omega_1'], actual['omega_2'], *params)
    E_raw = total_energy(pred_raw['theta_1'], pred_raw['theta_2'], pred_raw['omega_1'], pred_raw['omega_2'], *params)
    E_corr = total_energy(pred_corr['theta_1'], pred_corr['theta_2'], pred_corr['omega_1'], pred_corr['omega_2'], *params)
    ...
    axes[0].semilogy(t, np.maximum(err_raw, 1e-6), color='#ff6b6b', lw=1.6, label='uncorrected')
    axes[0].semilogy(t, np.maximum(err_corr, 1e-6), color='#2ca02c', lw=1.6, label='energy-corrected')
    axes[0].axhline(THRESHOLD, color='#888', linestyle='--', lw=1, label=f'{THRESHOLD} rad threshold')
    ...
    axes[1].plot(t, E_raw - E0, color='#ff6b6b', lw=1.4, label='uncorrected rollout', alpha=0.85)
    axes[1].plot(t, E_corr - E0, color='#2ca02c', lw=1.4, label='energy-corrected rollout', alpha=0.85)
    ...
    fig.suptitle('n=2: Physics-Informed Energy-Conserving Velocity Correction', fontweight='bold')
    plt.tight_layout()
    fig.savefig('figures/n2_energy_conservation_rollout.png', dpi=200, bbox_inches='tight')
