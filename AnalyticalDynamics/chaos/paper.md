...
and give two structural remedies with their costs. All numbers, figures and
negative results are reproducible from the accompanying code.
...
5. **Negative results reported as such** (§7, §8), including one intuitive
   stabiliser that makes matters 25× worse and one that stabilises by freezing
   the system.
...
31 initial conditions with $\theta_1(0) = 120°$ and the final link's angle swept
over $[0°, 3°]$ in $0.1°$ steps. The held-out condition is $2.05°$. Targets are
...
\Big(\bar y_r + \boldsymbol{\phi}(\mathbf{x})^\top \mathbf{a}_r\Big).
\tag{5}
$$
...
training), and **holdout-IC** (the never-trained $2.05°$ trajectory). Only the
...
The held-out initial condition $2.05°$ lies exactly midway between the trained
conditions $2.0°$ and $2.1°$. Consider therefore

$$
\hat{\boldsymbol\theta}_{\text{mid}}(t) = \tfrac{1}{2}\!\left(
\boldsymbol\theta^{(2.0°)}(t) + \boldsymbol\theta^{(2.1°)}(t)\right),
\tag{6}
$$
...
trajectory is smooth enough over a $0.1°$ grid that one-dimensional interpolation
suffices.
...
$$
s(t) = \max_i \left|\theta_i^{(2.1°)}(t) - \theta_i^{(2.0°)}(t)\right|
$$
...
from $0.1°$ to over $600°$ — the training set then contains two mutually
...
Refining $R$ from 40 to 300 improves the in-window RMSE from $22.0°$ to $3.11°$
while degrading the extrapolation from $443°$ to $338\,555°$ — three orders of
...
yields the family's best extrapolation ($146°$) at the cost of the worst
...
extrapolation RMSE from $1173°$ to $91.7°$. But it constrains the state's
...
$$
\boldsymbol\psi_1 = \frac{1}{\ell_1 D}
\begin{bmatrix}
\sin\theta_1 \\
\sin(\theta_1 - 2\theta_2) \\
\omega_2^2 \sin\Delta \\
...
and analogously $\boldsymbol\psi_2 \in \mathbb{R}^4$. Crucially, the damping
...
holdout spans $225°$ and $937°$ in its two angles, so the original study's RMSE of
0.26 corresponds to roughly $177°$. We report degrees throughout alongside scaled
values.
...
references unchanged to $0.00°$ over 20 s, but moves the frictionless ones by
...
recommendations are: never index the model by time if a horizon is wanted; give
the consequents the structure that makes dissipation a first-class term rather
than a residual; report a zero-parameter baseline; and separate model error from
integrator error before claiming either.
