...
PHYSICS_BASIS_COLS = [
    'sin_th1', 'sin_th1_minus_2th2',
    'sinD_om1sq', 'sinD_om2sq',
    'sinD_om1sq_cosD', 'sinD_om2sq_cosD',
    'sinD_costh1', 'cosD', 'cos2D',
]
...
if __name__ == '__main__':
    dt = 0.01
    print("Generating n=2 training set (same 16-trajectory scenario as the original study)...")
    ...
    axes[0].plot(t[valid.values], predicted['theta_1'].values[valid.values], color='#ff1744', lw=1.6,
                 label='Predicted (physics-basis features)', alpha=0.85)
    ...
    axes[1].semilogy(t, np.maximum(err_theta1, 1e-6), color='#9467bd', lw=1.6,
    ...
    fig.suptitle('n=2: Physics-Inspired (Lagrangian-Structured) Consequent Features', fontweight='bold')
    plt.tight_layout()
    fig.savefig(FIG_DIR / 'n2_physics_informed_rollout.png', dpi=200, bbox_inches='tight')
