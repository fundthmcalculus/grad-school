...
def check_n2_matches_hand_derivation(n_trials=200, seed=42):
    ...
    print(f"n=2 vs hand-derived DoublePendulum: max abs error over {n_trials} random "
          f"states = {max_err:.3e}")
    return max_err
...
def validate_n(n, theta0_deg, dt=0.001, duration=8.0, g=9.81):
    ...
    print(f"n={n}: build {build_s:.2f}s, integrate {integrate_s:.2f}s ({len(tspan)} steps), "
          f"energy drift {drift:.3e} J ({pct:.5f}% of |E0|)")

    return dict(n=n, build_s=build_s, integrate_s=integrate_s, drift=drift, pct=pct,
                theta=theta, omega=omega, t=tspan, m_vals=m_vals, l_vals=l_vals)
...
def plot_chain_snapshots(results_by_n):
    ...
    for ax, r in zip(axes, results_by_n):
        ...
        ax.set_aspect('equal')
        ax.set_title(f'n={n}: last-bob trace over {r["t"][-1]:.0f}s\n'
                     f'(final configuration in black)', fontsize=11)
        ax.grid(alpha=0.3)
    plt.tight_layout()
    fig.savefig(FIG_DIR / 'n_pendulum_chain_snapshots.png', dpi=200, bbox_inches='tight')
    plt.close(fig)
