...
def simulate(n, theta0_deg, m_vals=None, l_vals=None, g=9.81, dt=0.005, duration=15.0):
    ...
    print(f"  n={n}: energy drift {drift:.3e} J ({pct:.5f}% of |E0|) -- "
          f"{'OK' if pct < 1.0 else 'SUSPICIOUSLY LARGE'}")

    return dict(n=n, t=tspan, theta=theta, omega=omega, m_vals=m_vals, l_vals=l_vals,
                E=E, theta0_deg=theta0_deg)


def render_gif(sim, out_path, max_frames=320, fps=28, trail_len=60, figsize=(6, 6), dpi=110):
    n = sim['n']
    ...
    ax.set_title(f'{n}-Link Pendulum  ' + r'($\theta_0$=' + f'{sim["theta0_deg"]}' + r'$\degree$)',
                 color='white', fontsize=12, fontweight='bold')
    time_text = ax.text(0.02, 0.02, '', transform=ax.transAxes, color='#aaaaaa', fontsize=9)

    trail, = ax.plot([], [], '-', color='#00d4ff', linewidth=1.1, alpha=0.55)
    chain_line, = ax.plot([], [], 'o-', color='#e0e0e0', lw=2.2, ms=5, markerfacecolor='white')
    ...
    def update(frame):
        ...
        trail.set_data(x[t_start:i + 1:max(1, step // 2 + 1), -1],
